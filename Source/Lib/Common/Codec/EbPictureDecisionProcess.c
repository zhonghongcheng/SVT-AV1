...
#if sc_rtime_presets
#if M2_PIC_DEPTH || M3_SC_NSQ_DEPTH
...
#elif M3_NSQ_MDC_CANDIDATE && !M2_MDC
    else if (picture_control_set_ptr->enc_mode <= ENC_M3)
    ...
    // NSQ_SEARCH_FULL                                Allow NSQ Intra-FULL and Inter-FULL
        ...
        if (MR_MODE)
            ...
            if (picture_control_set_ptr->is_used_as_reference_flag)
                ...
                picture_control_set_ptr->nsq_search_level = NSQ_SEARCH_LEVEL1;
#elif M3_NSQ_MDC_CANDIDATE && !M3_PIC_DEPTH_NSQ
#if SHIFT_M4_TO_M3
        else if (picture_control_set_ptr->enc_mode <= ENC_M2)
#else
        else if (picture_control_set_ptr->enc_mode <= ENC_M3)
...
#elif M0_PALETTE_IN_M2
            picture_control_set_ptr->enc_mode <= ENC_M2 ? 6 : 0;
#else
...
#if rtime_presets
#if M1_INTRAPRED
        if ((picture_control_set_ptr->enc_mode <= ENC_M2) || (picture_control_set_ptr->enc_mode <= ENC_M2 && picture_control_set_ptr->temporal_layer_index == 0))
#else
#if M2_INTRAPRED
    ...
    picture_control_set_ptr->skip_sub_blks =   0;
      ...
      if (picture_control_set_ptr->sc_content_detected)
        ...
        if (sequence_control_set_ptr->compound_mode)
            ...
            if (picture_control_set_ptr->sc_content_detected)
#if M0_COMPOUND_MODE
                picture_control_set_ptr->compound_mode = (picture_control_set_ptr->enc_mode <= ENC_M1) ? 2 : 0;
#else
