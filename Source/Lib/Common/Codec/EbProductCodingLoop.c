...
void perform_fast_loop(
    ...
    while (fastLoopCandidateIndex >= fast_candidate_start_index)
    {
        ...
        if (candidate_ptr->distortion_ready) {
            ...
            // Fast Cost
#if SHUT_RATE_MD_STAGE
            if (context_ptr->md_staging_mode  && context_ptr->md_stage == MD_STAGE_0 && (context_ptr->target_class == (picture_control_set_ptr->enc_mode == ENC_M0 ? CAND_CLASS_1_M0 : CAND_CLASS_1) || context_ptr->target_class == (picture_control_set_ptr->enc_mode == ENC_M0 ? CAND_CLASS_2_M0 : CAND_CLASS_2) || context_ptr->target_class == (picture_control_set_ptr->enc_mode == ENC_M0 ? CAND_CLASS_3_M0 : CAND_CLASS_3)) && context_ptr->bypass_stage1[context_ptr->target_class] == EB_FALSE)
    ...
    uint32_t                 fastCandidateTotalCount)
{
    ...
    */
    ...
    for (uint8_t class_idx = 0; class_idx < CAND_CLASS_TOTAL; class_idx++) {
        context_ptr->fast1_cand_count[class_idx] = 0;
        context_ptr->md_stage_2_count[class_idx] = 0;
        context_ptr->md_stage_3_count[class_idx] = 0;
