    ...
    uint32_t                 fastCandidateTotalCount)
{
    ...
    if (context_ptr->md_staging_mode == MD_STAGING_MODE_1 && context_ptr->pd_pass == EB_TRUE)
#else
...
void md_stage_0(
    ...
    context_ptr->md_staging_skip_interpolation_search = (context_ptr->md_staging_mode == MD_STAGING_MODE_1 || context_ptr->pd_pass == EB_FALSE) ? EB_TRUE : picture_control_set_ptr->parent_pcs_ptr->interpolation_search_level >= IT_SEARCH_FAST_LOOP_UV_BLIND ? EB_FALSE : EB_TRUE;
#else
    ...
    context_ptr->md_staging_skip_inter_chroma_pred = ((context_ptr->md_staging_mode == MD_STAGING_MODE_1 && context_ptr->target_class != CAND_CLASS_0 && context_ptr->target_class != CAND_CLASS_6 && context_ptr->target_class != CAND_CLASS_7) || context_ptr->pd_pass == EB_FALSE) ? EB_TRUE : EB_FALSE;
#else
    ...
    EbAsm                  asm_type)
{
    ...
    for (fullLoopCandidateIndex = 0; fullLoopCandidateIndex < fullCandidateTotalCount; ++fullLoopCandidateIndex) {
        ...
        context_ptr->md_staging_skip_full_pred = (context_ptr->md_staging_mode == MD_STAGING_MODE_0 && picture_control_set_ptr->parent_pcs_ptr->interpolation_search_level != IT_SEARCH_FULL_LOOP) || (context_ptr->pd_pass == EB_FALSE);
#else
        ...
        context_ptr->md_staging_skip_atb = context_ptr->pd_pass == EB_TRUE ? context_ptr->coeff_based_skip_atb : EB_TRUE;
#else
        ...
        context_ptr->md_staging_skip_full_chroma = context_ptr->pd_pass == EB_TRUE ? EB_FALSE : EB_TRUE;
        context_ptr->md_staging_skip_rdoq = context_ptr->pd_pass == EB_TRUE ? EB_FALSE : EB_TRUE;
#else
    ...
    ModeDecisionCandidateBuffer    *bestcandidate_buffers[5])
{
    ...
    is_nsq_table_used = picture_control_set_ptr->parent_pcs_ptr->sc_content_detected || picture_control_set_ptr->enc_mode <= ENC_M1 ? EB_FALSE : is_nsq_table_used;
        ...
        is_nsq_table_used, picture_control_set_ptr->parent_pcs_ptr->nsq_max_shapes_md, context_ptr, is_complete_sb))
    {
        ...
        int allow_ii = (context_ptr->pd_pass == EB_TRUE) ? is_interintra_allowed_bsize(context_ptr->blk_geom->bsize) : 0;
#else
