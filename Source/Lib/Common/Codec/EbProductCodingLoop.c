    ...
    EbBool                               use_ssd)
{
    ...
    if (candidate_ptr->type == INTER_MODE && candidateBuffer->candidate_ptr->is_compound)
        determine_compound_mode(
            ...
            to_svt_compound_lut[candidateBuffer->candidate_ptr->interinter_comp.type]);
#endif
    ...
    if (use_ssd) {
        ...
        lumaFastDistortion = (nxm_sad_kernel_sub_sampled(
            input_picture_ptr->buffer_y + inputOriginIndex,
            input_picture_ptr->stride_y,
            prediction_ptr->buffer_y + cuOriginIndex,
            prediction_ptr->stride_y,
    ...
    while (fastLoopCandidateIndex >= fast_candidate_start_index)
    {
        ...
        if (!candidate_ptr->distortion_ready || fastLoopCandidateIndex == bestFirstFastCostSearchCandidateIndex) {
            ...
            if (use_ssd) {
                ...
                assert((context_ptr->blk_geom->bwidth >> 3) < 17);
                candidateBuffer->candidate_ptr->luma_fast_distortion = lumaFastDistortion = (nxm_sad_kernel_sub_sampled(
                    input_picture_ptr->buffer_y + inputOriginIndex,
                    input_picture_ptr->stride_y,
                    prediction_ptr->buffer_y + cuOriginIndex,
                    prediction_ptr->stride_y,
    ...
    */
    ...
    if (picture_control_set_ptr->enc_mode >= ENC_M2 && picture_control_set_ptr->enc_mode <= ENC_M4) {
        ...
        if (context_ptr->md_staging_mode == 0 && picture_control_set_ptr->enc_mode <= ENC_M6) {
            context_ptr->fast_cand_count[CAND_CLASS_0] = (picture_control_set_ptr->slice_type == I_SLICE) ? 8 : (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) ? 3 : 1;

            if (context_ptr->combine_class12) {
                context_ptr->fast_cand_count[CAND_CLASS_1] = (picture_control_set_ptr->slice_type == I_SLICE) ? 0 : (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) ? 5 : 3;
                context_ptr->fast_cand_count[CAND_CLASS_2] = (picture_control_set_ptr->slice_type == I_SLICE) ? 0 : (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) ? 1 : 1;

            }
            else {
                context_ptr->fast_cand_count[CAND_CLASS_1] = (picture_control_set_ptr->slice_type == I_SLICE) ? 0 : (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) ? 2 : 1;
                context_ptr->fast_cand_count[CAND_CLASS_2] = (picture_control_set_ptr->slice_type == I_SLICE) ? 0 : (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) ? 2 : 1;
                context_ptr->fast_cand_count[CAND_CLASS_3] = (picture_control_set_ptr->slice_type == I_SLICE) ? 0 : (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) ? 1 : 1;
            }

            context_ptr->md_stage_2_count[CAND_CLASS_0] = context_ptr->fast_cand_count[CAND_CLASS_0];
            context_ptr->md_stage_2_count[CAND_CLASS_1] = context_ptr->fast_cand_count[CAND_CLASS_1];
            context_ptr->md_stage_2_count[CAND_CLASS_2] = context_ptr->fast_cand_count[CAND_CLASS_2];

            if (!context_ptr->combine_class12)
                context_ptr->md_stage_2_count[CAND_CLASS_3] = context_ptr->fast_cand_count[CAND_CLASS_3];
            ...
            context_ptr->fast_cand_count[CAND_CLASS_0] = (picture_control_set_ptr->slice_type == I_SLICE) ? 6 : (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) ? 2 : 1;

            if (context_ptr->combine_class12) {
                context_ptr->fast_cand_count[CAND_CLASS_1] = (picture_control_set_ptr->slice_type == I_SLICE) ? 0 : (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) ? 4 : 2;
                context_ptr->fast_cand_count[CAND_CLASS_2] = (picture_control_set_ptr->slice_type == I_SLICE) ? 0 : (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) ? 1 : 1;
    ...
    context_ptr->md_stage_2_count[CAND_CLASS_0] = context_ptr->bypass_stage1[CAND_CLASS_0] ? context_ptr->fast_cand_count[CAND_CLASS_0] : context_ptr->md_stage_2_count[CAND_CLASS_0];
    context_ptr->md_stage_2_count[CAND_CLASS_1] = context_ptr->bypass_stage1[CAND_CLASS_1] ? context_ptr->fast_cand_count[CAND_CLASS_1] : context_ptr->md_stage_2_count[CAND_CLASS_1];
    context_ptr->md_stage_2_count[CAND_CLASS_2] = context_ptr->bypass_stage1[CAND_CLASS_2] ? context_ptr->fast_cand_count[CAND_CLASS_2] : context_ptr->md_stage_2_count[CAND_CLASS_2];
    context_ptr->md_stage_2_count[CAND_CLASS_3] = context_ptr->bypass_stage1[CAND_CLASS_3] ? context_ptr->fast_cand_count[CAND_CLASS_3] : context_ptr->md_stage_2_count[CAND_CLASS_3];
    ...
    if (context_ptr->combine_class12)
        context_ptr->fast_cand_count[CAND_CLASS_3] = context_ptr->md_stage_2_count[CAND_CLASS_3] = context_ptr->md_stage_3_count[CAND_CLASS_3] = 0;
    }
#else
void set_md_stage_counts(
    PictureControlSet       *picture_control_set_ptr,
        ...
        if (rf[1] == NONE_FRAME)
        {
            ...
            if (pa_me_distortion != 0 || context_ptr->predictive_me_level >= 5) {
                ...
                if (pa_me_distortion == 0)
                    ...
                    exit_predictive_me_sub_pel = ((((best_search_distortion - pa_me_distortion) * 100) / pa_me_distortion) < DEVIATION_TH) ?
                        EB_FALSE :
                        EB_TRUE;
                }
                    ...
                    if (context_ptr->predictive_me_level >= 2) {
                        ...
                        if (context_ptr->predictive_me_level == 3) {
                            if (best_search_mvx % 8 != 0 || best_search_mvy % 8 != 0) {
                                ...
                                if (best_search_mvx % 8 == 0)
                                    search_pattern = 2;
                                else // if(best_search_mvy % 8 == 0)
                                    search_pattern = 3;
...
#if BYPASS_USELESS_TX_SEARCH
    tx_search_skip_flag = (blk_geom->bwidth >= 32 || blk_geom->bheight >= 32) ? 1 : tx_search_skip_flag;
#else
    ...
    EbAsm                         asm_type)
{
    ...
    uint32_t tu_origin_index = txb_origin_x + (txb_origin_y * candidateBuffer->residual_ptr->stride_y);
    ...
    for (tx_type = txk_start; tx_type < txk_end; ++tx_type) {
        ...
        av1_quantize_inv_quantize(
            ...
            &(((int32_t*)context_ptr->trans_quant_buffers_ptr->tu_trans_coeff2_nx2_n_ptr->buffer_y)[context_ptr->txb_1d_offset]),
            NOT_USED_VALUE,
            &(((int32_t*)candidateBuffer->residual_quant_coeff_ptr->buffer_y)[context_ptr->txb_1d_offset]),
            &(((int32_t*)candidateBuffer->recon_coeff_ptr->buffer_y)[context_ptr->txb_1d_offset]),
            qp,
            context_ptr->blk_geom->tx_width[context_ptr->tx_depth][context_ptr->txb_itr],
            context_ptr->blk_geom->tx_height[context_ptr->tx_depth][context_ptr->txb_itr],
            context_ptr->blk_geom->txsize[context_ptr->tx_depth][context_ptr->txb_itr],
        ...
        candidateBuffer->candidate_ptr->quantized_dc[0][context_ptr->txb_itr] = (((int32_t*)candidateBuffer->residual_quant_coeff_ptr->buffer_y)[context_ptr->txb_1d_offset]);
    ...
    uint64_t                     *y_full_distortion)
{
    ...
    for (context_ptr->tx_depth = 0; context_ptr->tx_depth <= end_tx_depth; context_ptr->tx_depth++) {
...
#endif
#if ATB_INTER_SUPPORT // transform_type_uv
                // For Inter blocks, transform type of chroma follows luma transfrom type
                if (is_inter)
                    candidateBuffer->candidate_ptr->transform_type_uv = (context_ptr->txb_itr == 0) ? candidateBuffer->candidate_ptr->transform_type[context_ptr->txb_itr] : candidateBuffer->candidate_ptr->transform_type_uv;
#endif
     ...
     uint64_t cost = RDCOST(context_ptr->full_lambda, ((*y_coeff_bits) + tx_size_bits), y_full_distortion[DIST_CALC_RESIDUAL]);
        ...
        uint64_t tx_size_bits = 0;
#if !SHUT_TX_SIZE_RATE || ATB_RATE_UPGRADE_0
#if 0//ATB_RATE_UPGRADE_0 //--------
        if(1)
        //if (/*context_ptr->tx_depth && is_inter &&*/ candidateBuffer->candidate_ptr->y_has_coeff)
#else
    ...
    if (context_ptr->tx_depth == 0) {
        ...
        *y_coeff_bits = 0;
        ...
        for (context_ptr->txb_itr = 0; context_ptr->txb_itr < txb_count; context_ptr->txb_itr++) {
            ...
            // For Inter blocks, transform type of chroma follows luma transfrom type
            if (is_inter)
                candidateBuffer->candidate_ptr->transform_type_uv = best_tx_type_depth_0;
#endif
#endif
...
#if DISABLE_NSQ_TABLE
#if NSQ_TABLES_NREF
    is_nsq_table_used = ((picture_control_set_ptr->enc_mode <= ENC_M0 && picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) || (picture_control_set_ptr->enc_mode == ENC_M0 && picture_control_set_ptr->parent_pcs_ptr->sc_content_detected))
#else
    is_nsq_table_used = (picture_control_set_ptr->enc_mode == ENC_M0 || (picture_control_set_ptr->enc_mode == ENC_M0 && picture_control_set_ptr->parent_pcs_ptr->sc_content_detected))
#endif
        ? EB_FALSE : is_nsq_table_used;
...
#endif
    {
        ...
        for (cand_class_it = CAND_CLASS_0; cand_class_it < CAND_CLASS_TOTAL; cand_class_it++) {
            ...
            if (cand_class_it == CAND_CLASS_3 && context_ptr->fast_cand_count[cand_class_it]) {
                ...
                uint64_t class_0_to_class_2_dev = (context_ptr->best_cost_per_class[CAND_CLASS_0] < context_ptr->best_cost_per_class[CAND_CLASS_2] && context_ptr->best_cost_per_class[CAND_CLASS_0]) ?
