...
#if DISABLE_NSQ_TABLE
#if NSQ_TABLES_NREF
    is_nsq_table_used = ((picture_control_set_ptr->enc_mode <= ENC_M1 && picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag) || (picture_control_set_ptr->enc_mode == ENC_M0 && picture_control_set_ptr->parent_pcs_ptr->sc_content_detected))
#else
    is_nsq_table_used = (picture_control_set_ptr->enc_mode == ENC_M0 || (picture_control_set_ptr->enc_mode == ENC_M0 && picture_control_set_ptr->parent_pcs_ptr->sc_content_detected))
#endif
        ? EB_FALSE : is_nsq_table_used;
