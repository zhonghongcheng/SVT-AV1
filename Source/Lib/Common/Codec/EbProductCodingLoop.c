    ...
    ModeDecisionContext   *context_ptr)
{
    ...
    for (uint8_t uv_mode_count = 0; uv_mode_count < uv_mode_total_count; uv_mode_count++) {
        ...
        ModeDecisionCandidate         *candidate_ptr = candidate_buffer->candidate_ptr = &context_ptr->fast_candidate_array[uv_mode_count];
    ...
    for (uint8_t uv_mode_count = 0; uv_mode_count < MIN(uv_mode_total_count, uv_mode_nfl_count); uv_mode_count++) {
        ...
        ModeDecisionCandidate         *candidate_ptr = candidate_buffer->candidate_ptr = &context_ptr->fast_candidate_array[uv_cand_buff_indices[uv_mode_count]];
        uint16_t  cb_qp = context_ptr->qp;
        uint16_t  cr_qp = context_ptr->qp;
        uint64_t cb_coeff_bits = 0;
        uint64_t cr_coeff_bits = 0;
        ...
        coeff_rate[candidate_buffer->candidate_ptr->intra_chroma_mode][MAX_ANGLE_DELTA + candidate_ptr->angle_delta[PLANE_TYPE_UV]] = (int)(cb_coeff_bits + cr_coeff_bits);
        distortion[candidate_buffer->candidate_ptr->intra_chroma_mode][MAX_ANGLE_DELTA + candidate_ptr->angle_delta[PLANE_TYPE_UV]] = (int)(cbFullDistortion[DIST_CALC_RESIDUAL] + crFullDistortion[DIST_CALC_RESIDUAL]);
    }
