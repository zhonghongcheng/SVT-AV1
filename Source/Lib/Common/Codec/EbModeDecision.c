...
#define INCRMENT_CAND_TOTAL_COUNT(cnt)                                      \
    ...
    if (cnt >= MODE_DECISION_CANDIDATE_MAX_COUNT)                           \
        printf(                                                             \
            " ERROR: reaching limit for MODE_DECISION_CANDIDATE_MAX_COUNT " \
            "%i\n",                                                         \
            cnt);
#endif
 ...
 * Constants
 ********************************************/
// static uint32_t  AntiContouringIntraMode[11] = { EB_INTRA_PLANAR,
// EB_INTRA_DC, EB_INTRA_HORIZONTAL, EB_INTRA_VERTICAL,  EB_INTRA_MODE_2,
// EB_INTRA_MODE_6, EB_INTRA_MODE_14, EB_INTRA_MODE_18, EB_INTRA_MODE_22,
// EB_INTRA_MODE_30, EB_INTRA_MODE_34 };

const uint32_t parentIndex[85] = {
    0,  0,  0,  2,  2,  2,  2,  0,  7,  7,  7,  7,  0,  12, 12, 12, 12,
    0,  17, 17, 17, 17, 0,  0,  23, 23, 23, 23, 0,  28, 28, 28, 28, 0,
    33, 33, 33, 33, 0,  38, 38, 38, 38, 0,  0,  44, 44, 44, 44, 0,  49,
    49, 49, 49, 0,  54, 54, 54, 54, 0,  59, 59, 59, 59, 0,  0,  65, 65,
    65, 65, 0,  70, 70, 70, 70, 0,  75, 75, 75, 75, 0,  80, 80, 80, 80};
...
uint8_t get_list_idx(uint8_t ref_type) {
    if (ref_type == LAST_FRAME || ref_type == LAST2_FRAME ||
        ref_type == LAST3_FRAME || ref_type == GOLDEN_FRAME)
        return 0;
    else if (ref_type == BWDREF_FRAME || ref_type == ALTREF_FRAME ||
             ref_type == ALTREF2_FRAME)
        return 1;
    else
        return (INVALID_REF);
};
...
uint8_t get_ref_frame_idx(uint8_t list, uint8_t ref_type) {
    switch (list) {
    case 0:
        return (ref_type == LAST_FRAME
                    ? 0
                    : ref_type == LAST2_FRAME
                          ? 1
                          : ref_type == LAST3_FRAME
                                ? 2
                                : ref_type == GOLDEN_FRAME ? 3 : INVALID_REF);
    case 1:
        return (ref_type == BWDREF_FRAME
                    ? 0
                    : ref_type == ALTREF_FRAME
                          ? 1
                          : ref_type == ALTREF2_FRAME ? 2 : INVALID_REF);
...
MvReferenceFrame svt_get_ref_frame_type(uint8_t list, uint8_t ref_idx) {
    ...
    case 0:
        return (ref_idx == 0
                    ? LAST_FRAME
                    : ref_idx == 1 ? LAST2_FRAME
                                   : ref_idx == 2 ? LAST3_FRAME
                                                  : ref_idx == 3 ? GOLDEN_FRAME
                                                                 : INVALID_REF);
    case 1:
        return (ref_idx == 0
                    ? BWDREF_FRAME
                    : ref_idx == 1 ? ALTREF2_FRAME
                                   : ref_idx == 2 ? ALTREF_FRAME : INVALID_REF);
    default: return (INVALID_REF);
    }
#else
    switch (list) {
                    ...
                    : ref_idx == 1 ? LAST2_FRAME
                                   : ref_idx == 2 ? LAST3_FRAME
                                                  : ref_idx == 3 ? GOLDEN_FRAME
                                                                 : INVALID_REF);
    case 1:
        return (ref_idx == 0 ? BWDREF_FRAME
                             : ref_idx == 1 ? ALTREF_FRAME
                                            : ref_idx == 2 ? ALTREF2_FRAME
                                                           : INVALID_REF);
    default: return (INVALID_REF);
    }
#endif
...
void ChooseBestAv1MvPred(ModeDecisionContext *context_ptr,
                         struct MdRateEstimationContext *md_rate_estimation_ptr,
                         CodingUnit *cu_ptr, MvReferenceFrame ref_frame,
                         uint8_t is_compound,
...
EbErrorType mode_decision_candidate_buffer_ctor(
    ModeDecisionCandidateBuffer **buffer_dbl_ptr, uint64_t *fast_cost_ptr,
    uint64_t *full_cost_ptr, uint64_t *full_cost_skip_ptr,
    ...
    EB_MALLOC(ModeDecisionCandidateBuffer *,
              bufferPtr,
              sizeof(ModeDecisionCandidateBuffer),
              EB_N_PTR);
    *buffer_dbl_ptr = bufferPtr;
    ...
    pictureBufferDescInitData.buffer_enable_mask =
    ...
    doubleWidthPictureBufferDescInitData.buffer_enable_mask =
        PICTURE_BUFFER_DESC_FULL_MASK;
    ...
    ThirtyTwoWidthPictureBufferDescInitData.buffer_enable_mask =
        PICTURE_BUFFER_DESC_FULL_MASK;
    ...
    // Video Buffers
    return_error =
        eb_picture_buffer_desc_ctor((EbPtr *)&(bufferPtr->prediction_ptr),
                                    (EbPtr)&pictureBufferDescInitData);

    // Video Buffers
    return_error =
        eb_picture_buffer_desc_ctor((EbPtr *)&(bufferPtr->prediction_ptr_temp),
                                    (EbPtr)&pictureBufferDescInitData);
    ...
    if (return_error == EB_ErrorInsufficientResources)
        return EB_ErrorInsufficientResources;
    return_error = eb_picture_buffer_desc_ctor(
        (EbPtr *)&(bufferPtr->recon_ptr), (EbPtr)&pictureBufferDescInitData);
...
void RoundMv(ModeDecisionCandidate *candidateArray, uint32_t canTotalCnt) {
    candidateArray[canTotalCnt].motion_vector_xl0 =
        (candidateArray[canTotalCnt].motion_vector_xl0 + 2) & ~0x03;
    candidateArray[canTotalCnt].motion_vector_yl0 =
        (candidateArray[canTotalCnt].motion_vector_yl0 + 2) & ~0x03;

    candidateArray[canTotalCnt].motion_vector_xl1 =
        (candidateArray[canTotalCnt].motion_vector_xl1 + 2) & ~0x03;
    candidateArray[canTotalCnt].motion_vector_yl1 =
        (candidateArray[canTotalCnt].motion_vector_yl1 + 2) & ~0x03;
...
EbBool mrp_is_already_injected_mv_l0(ModeDecisionContext *context_ptr,
                                     int16_t mv_x, int16_t mv_y,
                                     uint8_t ref_type) {
    for (int inter_candidate_index = 0;
         inter_candidate_index < context_ptr->injected_mv_count_l0;
         inter_candidate_index++) {
        if (context_ptr->injected_mv_x_l0_array[inter_candidate_index] ==
                mv_x &&
            context_ptr->injected_mv_y_l0_array[inter_candidate_index] ==
                mv_y &&
            context_ptr->injected_ref_type_l0_array[inter_candidate_index] ==
                ref_type) {
            return (EB_TRUE);
        }
    }
...
EbBool mrp_is_already_injected_mv_l1(ModeDecisionContext *context_ptr,
                                     int16_t mv_x, int16_t mv_y,
                                     uint8_t ref_type) {
    for (int inter_candidate_index = 0;
         inter_candidate_index < context_ptr->injected_mv_count_l1;
         inter_candidate_index++) {
        if (context_ptr->injected_mv_x_l1_array[inter_candidate_index] ==
                mv_x &&
            context_ptr->injected_mv_y_l1_array[inter_candidate_index] ==
                mv_y &&
            context_ptr->injected_ref_type_l1_array[inter_candidate_index] ==
                ref_type) {
            return (EB_TRUE);
        }
    }
...
EbBool mrp_is_already_injected_mv_bipred(ModeDecisionContext *context_ptr,
                                         int16_t mv_x_l0, int16_t mv_y_l0,
                                         int16_t mv_x_l1, int16_t mv_y_l1,
                                         uint8_t ref_type) {
    for (int inter_candidate_index = 0;
         inter_candidate_index < context_ptr->injected_mv_count_bipred;
         inter_candidate_index++) {
        if (context_ptr->injected_mv_x_bipred_l0_array[inter_candidate_index] ==
                mv_x_l0 &&
            context_ptr->injected_mv_y_bipred_l0_array[inter_candidate_index] ==
                mv_y_l0 &&
            context_ptr->injected_mv_x_bipred_l1_array[inter_candidate_index] ==
                mv_x_l1 &&
            context_ptr->injected_mv_y_bipred_l1_array[inter_candidate_index] ==
                mv_y_l1 &&
            context_ptr
                    ->injected_ref_type_bipred_array[inter_candidate_index] ==
                ref_type) {
            return (EB_TRUE);
        }
    }
...
#else
EbBool is_already_injected_mv_l0(ModeDecisionContext *context_ptr, int16_t mv_x,
                                 int16_t mv_y) {
    for (int inter_candidate_index = 0;
         inter_candidate_index < context_ptr->injected_mv_count_l0;
         inter_candidate_index++) {
...
EbBool is_already_injected_mv_l1(ModeDecisionContext *context_ptr, int16_t mv_x,
                                 int16_t mv_y) {
    for (int inter_candidate_index = 0;
         inter_candidate_index < context_ptr->injected_mv_count_l1;
         inter_candidate_index++) {
...
EbBool is_already_injected_mv_bipred(ModeDecisionContext *context_ptr,
                                     int16_t mv_x_l0, int16_t mv_y_l0,
                                     int16_t mv_x_l1, int16_t mv_y_l1) {
    for (int inter_candidate_index = 0;
         inter_candidate_index < context_ptr->injected_mv_count_bipred;
...
EbErrorType SetMvpClipMVs(ModeDecisionCandidate *candidate_ptr,
                          uint32_t cu_origin_x, uint32_t cu_origin_y,
                          uint32_t pu_index, uint32_t tb_size,
...
void LimitMvOverBound(int16_t *mvx, int16_t *mvy, ModeDecisionContext *ctxtPtr,
    ...
    if ((int32_t)ctxtPtr->cu_origin_x + mvxF +
            (int32_t)ctxtPtr->blk_geom->bwidth >
        (int32_t)sCSet->luma_width)
        *mvx = (int16_t)(sCSet->luma_width - ctxtPtr->blk_geom->bwidth -
                         ctxtPtr->cu_origin_x);
    if ((int32_t)ctxtPtr->cu_origin_y + mvyF +
            (int32_t)ctxtPtr->blk_geom->bheight >
        (int32_t)sCSet->luma_height)
        *mvy = (int16_t)(sCSet->luma_height - ctxtPtr->blk_geom->bheight -
                         ctxtPtr->cu_origin_y);
...
void sort_fast_loop_candidates(struct ModeDecisionContext *context_ptr,
    ...
    for (uint8_t full_buffer_index = 0; full_buffer_index < buffer_total_count;
         full_buffer_index++) {
        if (*(buffer_ptr_array[full_buffer_index]->fast_cost_ptr) ==
            MAX_CU_COST)
            best_candidate_index_array[best_candidate_end_index--] =
                full_buffer_index;
        else
            best_candidate_index_array[best_candidate_start_index++] =
                full_buffer_index;
    }
    ...
    for (i = 0; i < fullReconCandidateCount - 1; ++i) {
        for (j = i + 1; j < fullReconCandidateCount; ++j) {
            if ((buffer_ptr_array[best_candidate_index_array[i]]
                     ->candidate_ptr->type == INTRA_MODE) &&
                (buffer_ptr_array[best_candidate_index_array[j]]
                     ->candidate_ptr->type == INTER_MODE)) {
                index = best_candidate_index_array[i];
                best_candidate_index_array[i] =
                    (uint8_t)best_candidate_index_array[j];
    ...
    for (i = 0; i < fullReconCandidateCount - 1; ++i) {
        for (j = i + 1; j < fullReconCandidateCount; ++j) {
            if (*(buffer_ptr_array[sorted_candidate_index_array[j]]
                      ->fast_cost_ptr) <
                *(buffer_ptr_array[sorted_candidate_index_array[i]]
                      ->fast_cost_ptr)) {
                index = sorted_candidate_index_array[i];
                sorted_candidate_index_array[i] =
                    (uint8_t)sorted_candidate_index_array[j];
    ...
    // tx search
    *ref_fast_cost =
        *(buffer_ptr_array[sorted_candidate_index_array[0]]->fast_cost_ptr);
#else
    // tx search
    for (i = 0; i < fullReconCandidateCount; i++) {
    ...
    for (i = 0; i < fullReconCandidateCount - 1; ++i) {
        for (j = i + 1; j < fullReconCandidateCount; ++j) {
            if (*(buffer_ptr_array[j]->fast_cost_ptr) <
                *(buffer_ptr_array[i]->fast_cost_ptr)) {
                index = sorted_candidate_index_array[i];
                sorted_candidate_index_array[i] =
                    (uint8_t)sorted_candidate_index_array[j];
                sorted_candidate_index_array[j] = (uint8_t)index;
...
int8_t ALLOW_REFINEMENT_FLAG[BIPRED_3x3_REFINMENT_POSITIONS] = {
    1, 0, 1, 0, 1, 0, 1, 0};
int8_t BIPRED_3x3_X_POS[BIPRED_3x3_REFINMENT_POSITIONS] = {
    -1, -1, 0, 1, 1, 1, 0, -1};
int8_t BIPRED_3x3_Y_POS[BIPRED_3x3_REFINMENT_POSITIONS] = {
    0, 1, 1, 1, 0, -1, -1, -1};
...
void Unipred3x3CandidatesInjection(
...
#endif
    PictureControlSet *picture_control_set_ptr,
    ModeDecisionContext *context_ptr, LargestCodingUnit *sb_ptr,
    uint32_t me_sb_addr, SsMeContext *inloop_me_context,
    EbBool use_close_loop_me, uint32_t close_loop_me_index,
#if !MEMORY_FOOTPRINT_OPT_ME_MV
    ...
    const MeLcuResults *me_results =
        picture_control_set_ptr->parent_pcs_ptr->me_results[me_sb_addr];
    uint8_t total_me_cnt =
        me_results->total_me_candidate_index[context_ptr->me_block_offset];
    ...
    const MeLcuResults *me_results =
        picture_control_set_ptr->parent_pcs_ptr->me_results[me_sb_addr];
    uint8_t total_me_cnt =
        me_results->total_me_candidate_index[me2Nx2NTableOffset];
    const MeCandidate *me_block_results =
        me_results->me_candidate[me2Nx2NTableOffset];
#endif
#else
    MeCuResults *mePuResult = &picture_control_set_ptr->parent_pcs_ptr
                                   ->me_results[me_sb_addr][me2Nx2NTableOffset];
#endif
    ModeDecisionCandidate *candidateArray = context_ptr->fast_candidate_array;
    EbBool isCompoundEnabled =
        (picture_control_set_ptr->parent_pcs_ptr->reference_mode ==
         SINGLE_REFERENCE)
            ? 0
            : 1;
    ...
    // (8 Best_L0 neighbors)
#if MD_INJECTION
    // const MeLcuResults_t *meResults =
    // pictureControlSetPtr->ParentPcsPtr->meResultsPtr[lcuAddr];
    for (uint8_t me_candidate_index = 0; me_candidate_index < total_me_cnt;
         ++me_candidate_index) {
        const MeCandidate *me_block_results_ptr =
            &me_block_results[me_candidate_index];
...
#endif
            for (bipredIndex = 0; bipredIndex < BIPRED_3x3_REFINMENT_POSITIONS;
                 ++bipredIndex) {
                /**************
                NEWMV L0
                ************* */
                ...
                int16_t to_inject_mv_x =
                    use_close_loop_me
                        ? (inloop_me_context
                               ->inloop_me_mv[0][0][close_loop_me_index][0] +
                           BIPRED_3x3_X_POS[bipredIndex])
                              << 1
                        : (me_results
                ...
                int16_t to_inject_mv_y =
                    use_close_loop_me
                        ? (inloop_me_context
                               ->inloop_me_mv[0][0][close_loop_me_index][1] +
                           BIPRED_3x3_Y_POS[bipredIndex])
                              << 1
                        : (me_results
                               ->me_mv_array[context_ptr->me_block_offset]
                                            [list0_ref_index]
        ...
        int16_t to_inject_mv_x =
            use_close_loop_me
                ? (inloop_me_context
                   ...
                   BIPRED_3x3_X_POS[bipredIndex])
                      << 1
                : (mePuResult->x_mv_l0 + BIPRED_3x3_X_POS[bipredIndex]) << 1;
        int16_t to_inject_mv_y =
            use_close_loop_me
                ? (inloop_me_context
                       ->inloop_me_mv[0][0][close_loop_me_index][1] +
...
#else
        if (context_ptr->injected_mv_count_l0 == 0 ||
            is_already_injected_mv_l0(
                context_ptr, to_inject_mv_x, to_inject_mv_y) == EB_FALSE) {
#endif
                    ...
                    candidateArray[canTotalCnt].motion_mode =
                        SIMPLE_TRANSLATION;
                    ...
                    candidateArray[canTotalCnt].ref_frame_type =
                        svt_get_ref_frame_type(REF_LIST_0, list0_ref_index);
                    candidateArray[canTotalCnt].ref_frame_index_l0 =
                        list0_ref_index;
            ...
            candidateArray[canTotalCnt].transform_type[PLANE_TYPE_UV] = DCT_DCT;
                    ...
                    candidateArray[canTotalCnt]
                        .motion_vector_pred_x[REF_LIST_0] =
                        bestPredmv[0].as_mv.col;
                    candidateArray[canTotalCnt]
                        .motion_vector_pred_y[REF_LIST_0] =
                        bestPredmv[0].as_mv.row;
#if CHECK_CAND
                    INCRMENT_CAND_TOTAL_COUNT(canTotalCnt);
#else
            ++canTotalCnt;
#endif
                    context_ptr->injected_mv_x_l0_array
                        [context_ptr->injected_mv_count_l0] = to_inject_mv_x;
                    context_ptr->injected_mv_y_l0_array
                        [context_ptr->injected_mv_count_l0] = to_inject_mv_y;
#if MRP_DUPLICATION_FIX
    ...
    for (uint8_t me_candidate_index = 0; me_candidate_index < total_me_cnt;
        ...
        if (inter_direction == 1) {
                ...
                if (isCompoundEnabled) {
                    ...
                    ************* */
                    ...
                    int16_t to_inject_mv_x =
                        use_close_loop_me
                            ? (inloop_me_context
                                   ->inloop_me_mv[1][0][close_loop_me_index]
                                                 [0] +
                               BIPRED_3x3_X_POS[bipredIndex])
                                  << 1
                            : (me_results
                        ...
                        use_close_loop_me
                            ? (inloop_me_context
                               ...
                               BIPRED_3x3_X_POS[bipredIndex])
                                  << 1
                            : (me_results
                                   ->me_mv_array[me2Nx2NTableOffset]
                                                [4 + list1_ref_index]
                                   .x_mv +
                               BIPRED_3x3_X_POS[bipredIndex])
                                  << 1;
            ...
            int16_t to_inject_mv_x =
                use_close_loop_me
                    ? (inloop_me_context
                           ->inloop_me_mv[1][0][close_loop_me_index][0] +
                       BIPRED_3x3_X_POS[bipredIndex])
                          << 1
                    : (mePuResult->x_mv_l1 + BIPRED_3x3_X_POS[bipredIndex])
                          << 1;
            int16_t to_inject_mv_y =
                use_close_loop_me
                    ? (inloop_me_context
                           ->inloop_me_mv[1][0][close_loop_me_index][1] +
                       BIPRED_3x3_Y_POS[bipredIndex])
                          << 1
                    : (mePuResult->y_mv_l1 + BIPRED_3x3_Y_POS[bipredIndex])
                          << 1;
#endif
#if MRP_DUPLICATION_FIX
                    uint8_t to_inject_ref_type =
                    ...
                    if (context_ptr->injected_mv_count_l1 == 0 ||
                        mrp_is_already_injected_mv_l1(context_ptr,
                                                      to_inject_mv_x,
                                                      to_inject_mv_y,
                                                      to_inject_ref_type) ==
                            EB_FALSE) {
            ...
            if (context_ptr->injected_mv_count_l1 == 0 ||
                is_already_injected_mv_l1(
                    context_ptr, to_inject_mv_x, to_inject_mv_y) == EB_FALSE) {
                        ...
                        candidateArray[canTotalCnt].ref_frame_index_l1 =
                            list1_ref_index;
#else
                candidateArray[canTotalCnt].ref_frame_type = BWDREF_FRAME;
                        ...
                        candidateArray[canTotalCnt].transform_type_uv = DCT_DCT;
#else
                candidateArray[canTotalCnt].transform_type[PLANE_TYPE_Y] =
                    DCT_DCT;
                candidateArray[canTotalCnt].transform_type[PLANE_TYPE_UV] =
                    DCT_DCT;
                        ...
                        candidateArray[canTotalCnt]
                            .motion_vector_pred_x[REF_LIST_1] =
                            bestPredmv[0].as_mv.col;
                        candidateArray[canTotalCnt]
                            .motion_vector_pred_y[REF_LIST_1] =
                            bestPredmv[0].as_mv.row;
#if CHECK_CAND
                        INCRMENT_CAND_TOTAL_COUNT(canTotalCnt);
#else
                ++canTotalCnt;
#endif
                        context_ptr->injected_mv_x_l1_array
                            [context_ptr->injected_mv_count_l1] =
                            to_inject_mv_x;
                        context_ptr->injected_mv_y_l1_array
                            [context_ptr->injected_mv_count_l1] =
                            to_inject_mv_y;
#if MRP_DUPLICATION_FIX
   ...
   ************* */
        ...
        for (uint8_t me_candidate_index = 0; me_candidate_index < total_me_cnt;
            ...
            if (inter_direction == 2) {
                ...
                for (bipredIndex = 0;
                     bipredIndex < BIPRED_3x3_REFINMENT_POSITIONS;
                     ++bipredIndex) {
                    ...
                    int16_t to_inject_mv_x_l0 =
                        use_close_loop_me
                            ? inloop_me_context
                                      ->inloop_me_mv[0][0][close_loop_me_index]
                                                    [0]
                                  << 1
                            : me_results
                                      ->me_mv_array[context_ptr
                                                        ->me_block_offset]
                                                   [list0_ref_index]
                                      .x_mv
                                  << 1;
                        ...
                        use_close_loop_me
                            ? inloop_me_context
                                      ->inloop_me_mv[0][0][close_loop_me_index]
                                                    [0]
                                  << 1
                            : me_block_results_ptr->x_mv_l0 << 1;
                    int16_t to_inject_mv_y_l0 =
                        use_close_loop_me
                            ? inloop_me_context
                                      ->inloop_me_mv[0][0][close_loop_me_index]
                                                    [1]
                                  << 1
                            : me_block_results_ptr->y_mv_l0 << 1;
                    int16_t to_inject_mv_x_l1 =
                        use_close_loop_me
                            ? ((inloop_me_context
                                    ->inloop_me_mv[1][0][close_loop_me_index]
            ...
            int16_t to_inject_mv_x_l0 =
                use_close_loop_me
                    ? inloop_me_context
                              ->inloop_me_mv[0][0][close_loop_me_index][0]
                          << 1
                    : mePuResult->x_mv_l0 << 1;
            int16_t to_inject_mv_y_l0 =
                use_close_loop_me
                    ? inloop_me_context
                              ->inloop_me_mv[0][0][close_loop_me_index][1]
                          << 1
                    : mePuResult->y_mv_l0 << 1;
            int16_t to_inject_mv_x_l1 =
                use_close_loop_me
                    ? ((inloop_me_context
                            ->inloop_me_mv[1][0][close_loop_me_index][0] +
            ...
            int16_t to_inject_mv_y_l1 =
                use_close_loop_me
                    ...
                    rf[0] = svt_get_ref_frame_type(
                        me_block_results_ptr->ref0_list, list0_ref_index);
                    rf[1] = svt_get_ref_frame_type(
                        me_block_results_ptr->ref1_list, list1_ref_index);
#else
...
#else
            if (context_ptr->injected_mv_count_bipred == 0 ||
                is_already_injected_mv_bipred(context_ptr,
                                              to_inject_mv_x_l0,
                                              to_inject_mv_y_l0,
                                              to_inject_mv_x_l1,
                                              to_inject_mv_y_l1) == EB_FALSE) {
#endif
                        candidateArray[canTotalCnt].type = INTER_MODE;
                        candidateArray[canTotalCnt].distortion_ready = 0;
                        candidateArray[canTotalCnt].use_intrabc = 0;
                ...
                candidateArray[canTotalCnt].transform_type[PLANE_TYPE_UV] =
                    DCT_DCT;
                        ...
                        candidateArray[canTotalCnt]
                            .motion_vector_pred_x[REF_LIST_1] =
                            bestPredmv[1].as_mv.col;
                        candidateArray[canTotalCnt]
                            .motion_vector_pred_y[REF_LIST_1] =
                            bestPredmv[1].as_mv.row;
#if CHECK_CAND
                        INCRMENT_CAND_TOTAL_COUNT(canTotalCnt);
#else
                ++canTotalCnt;
                        ...
                        use_close_loop_me
                            ? inloop_me_context
                                      ->inloop_me_mv[1][0][close_loop_me_index]
                                                    [0]
                                  << 1
                            : me_block_results_ptr->x_mv_l1 << 1;
                    int16_t to_inject_mv_y_l1 =
                        use_close_loop_me
                            ? inloop_me_context
                                      ->inloop_me_mv[1][0][close_loop_me_index]
                                                    [1]
                                  << 1
                            : me_block_results_ptr->y_mv_l1 << 1;
#endif
#else
            int16_t to_inject_mv_x_l0 =
                use_close_loop_me
                    ? (inloop_me_context
                       ...
                       BIPRED_3x3_X_POS[bipredIndex])
                          << 1
                    : (mePuResult->x_mv_l0 + BIPRED_3x3_X_POS[bipredIndex])
                          << 1;
            int16_t to_inject_mv_y_l0 =
                use_close_loop_me
                    ? (inloop_me_context
                       ...
                       BIPRED_3x3_Y_POS[bipredIndex])
                          << 1
                    : (mePuResult->y_mv_l0 + BIPRED_3x3_Y_POS[bipredIndex])
                          << 1;
            int16_t to_inject_mv_x_l1 =
                use_close_loop_me
                    ? inloop_me_context
                              ->inloop_me_mv[1][0][close_loop_me_index][0]
                          << 1
                    : mePuResult->x_mv_l1 << 1;
            int16_t to_inject_mv_y_l1 =
                use_close_loop_me
                    ? inloop_me_context
                              ->inloop_me_mv[1][0][close_loop_me_index][1]
                          << 1
                    : mePuResult->y_mv_l1 << 1;
#endif
...
void eighth_pel_unipred_refinement(
    const SequenceControlSet *sequence_control_set_ptr,
    PictureControlSet *picture_control_set_ptr,
    ModeDecisionContext *context_ptr, uint32_t me_sb_addr,
    SsMeContext *inloop_me_context, EbBool use_close_loop_me,
    uint32_t close_loop_me_index, uint32_t *candidateTotalCnt) {
    uint32_t bipredIndex;
    uint32_t canTotalCnt = (*candidateTotalCnt);
    const MeLcuResults *me_results =
        picture_control_set_ptr->parent_pcs_ptr->me_results[me_sb_addr];
                    ...
                    ************* */
                    ...
                    if (context_ptr->injected_mv_count_l1 == 0 ||
                        ...
                        candidateArray[canTotalCnt].ref_frame_index_l1 =
                            list1_ref_index;
                        candidateArray[canTotalCnt]
                            .transform_type[PLANE_TYPE_Y] = DCT_DCT;
                        candidateArray[canTotalCnt]
                            .transform_type[PLANE_TYPE_UV] = DCT_DCT;
...
uint8_t GetMaxDrlIndex(uint8_t refmvCnt, PredictionMode mode) {
    ...
    if (mode == NEARMV || mode == NEAR_NEARMV || mode == NEAR_NEWMV ||
        mode == NEW_NEARMV) {
        if (refmvCnt < 3)
            maxDrl = 1;
        else if (refmvCnt == 3)
...
void inject_mvp_candidates_II(struct ModeDecisionContext *context_ptr,
                              PictureControlSet *picture_control_set_ptr,
                              CodingUnit *cu_ptr, MvReferenceFrame ref_pair,
                              uint32_t *candTotCnt) {
    ...
    if (rf[1] == NONE_FRAME) {
        ...
        // NEAREST
        int16_t to_inject_mv_x =
            context_ptr->cu_ptr->ref_mvs[frame_type][0].as_mv.col;
        int16_t to_inject_mv_y =
            context_ptr->cu_ptr->ref_mvs[frame_type][0].as_mv.row;
        ...
        if (inj_mv) {
            ...
            candidateArray[canIdx].ref_frame_index_l0 =
                (list_idx == 0) ? ref_idx : -1;
            candidateArray[canIdx].ref_frame_index_l1 =
            ...
            if (list_idx == 0) {
                ...
                context_ptr->injected_mv_x_l1_array
                    [context_ptr->injected_mv_count_l1] = to_inject_mv_x;
                context_ptr->injected_mv_y_l1_array
                    [context_ptr->injected_mv_count_l1] = to_inject_mv_y;
                context_ptr->injected_ref_type_l1_array
...
#endif
        {
            // NEAREST_NEAREST
            int16_t to_inject_mv_x_l0 =
                context_ptr->md_local_cu_unit[context_ptr->blk_geom->blkidx_mds]
            ...
            if (inj_mv) {
                ...
                candidateArray[canIdx].merge_flag =
                    picture_control_set_ptr->parent_pcs_ptr
                            ...
                            (rf[0] == picture_control_set_ptr->parent_pcs_ptr
                                              ->skip_mode_info.ref_frame_idx_0 +
                                          1) &&
                            (rf[1] == picture_control_set_ptr->parent_pcs_ptr
            ...
            // NEAR_NEAR
            maxDrlIndex =
                GetMaxDrlIndex(xd->ref_mv_count[ref_pair], NEAR_NEARMV);
            for (drli = 0; drli < maxDrlIndex; drli++) {
                get_av1_mv_pred_drl(context_ptr,
                                    cu_ptr,
                                    ref_pair,
                ...
                if (inj_mv) {
                    ...
                    candidateArray[canIdx].transform_type_uv = DCT_DCT;
#else
                    candidateArray[canIdx].transform_type[PLANE_TYPE_Y] =
                        DCT_DCT;
                    candidateArray[canIdx].transform_type[PLANE_TYPE_UV] =
                        DCT_DCT;
#endif
...
void InjectAv1MvpCandidates(struct ModeDecisionContext *context_ptr,
                            CodingUnit *cu_ptr, MvReferenceFrame *ref_frames,
                            PictureControlSet *picture_control_set_ptr,
                            uint32_t lcuAddr,
#if !M8_SKIP_BLK
                            uint32_t leaf_index,
#endif
                            EbBool allow_bipred, uint32_t *candTotCnt) {
...
// MRP_DUPLICATION_FIX (x12)
    ...
    isCompoundEnabled = (context_ptr->blk_geom->bwidth == 4 ||
                         context_ptr->blk_geom->bheight == 4)
                            ? EB_FALSE
                            : isCompoundEnabled;
#endif
    ...
    // NEAREST_L0
    int16_t to_inject_mv_x =
        context_ptr->cu_ptr->ref_mvs[LAST_FRAME][0].as_mv.col;
    int16_t to_inject_mv_y =
        context_ptr->cu_ptr->ref_mvs[LAST_FRAME][0].as_mv.row;
    if (context_ptr->injected_mv_count_l0 == 0 ||
        is_already_injected_mv_l0(
            context_ptr, to_inject_mv_x, to_inject_mv_y) == EB_FALSE) {
        candidateArray[canIdx].type = INTER_MODE;
    ...
    if (context_ptr->near_mv_injection) {
        ...
        for (drli = 0; drli < maxDrlIndex; drli++) {
            ...
            if (context_ptr->injected_mv_count_l0 == 0 ||
                ...
                candidateArray[canIdx].use_intrabc = 0;
                candidateArray[canIdx].merge_flag = EB_FALSE;
                candidateArray[canIdx].prediction_direction[0] =
                    UNI_PRED_LIST_0;
                candidateArray[canIdx].is_new_mv = 0;
                candidateArray[canIdx].is_zero_mv = 0;
                candidateArray[canIdx].motion_vector_xl0 = to_inject_mv_x;
    ...
    if (isCompoundEnabled) {
        int16_t to_inject_mv_x =
            context_ptr->cu_ptr->ref_mvs[BWDREF_FRAME][0].as_mv.col;
        int16_t to_inject_mv_y =
            context_ptr->cu_ptr->ref_mvs[BWDREF_FRAME][0].as_mv.row;
        if (context_ptr->injected_mv_count_l1 == 0 ||
            is_already_injected_mv_l1(
                context_ptr, to_inject_mv_x, to_inject_mv_y) == EB_FALSE) {
            // NEAREST_L1
        ...
        if (context_ptr->near_mv_injection) {
            ...
            // NEAR_L1
            maxDrlIndex =
                GetMaxDrlIndex(xd->ref_mv_count[BWDREF_FRAME], NEARMV);
            ...
            for (drli = 0; drli < maxDrlIndex; drli++) {
                ...
                int16_t to_inject_mv_y = nearmv[0].as_mv.row;
                if (context_ptr->injected_mv_count_l1 == 0 ||
                    is_already_injected_mv_l1(context_ptr,
                                              to_inject_mv_x,
                                              to_inject_mv_y) == EB_FALSE) {
                    candidateArray[canIdx].type = INTER_MODE;
                    candidateArray[canIdx].inter_mode = NEARMV;
                    candidateArray[canIdx].pred_mode = NEARMV;
                    candidateArray[canIdx].motion_mode = SIMPLE_TRANSLATION;
...
#endif
        {
            ...
            int16_t to_inject_mv_x_l0 =
                context_ptr->md_local_cu_unit[context_ptr->blk_geom->blkidx_mds]
                    .ed_ref_mv_stack[LAST_BWD_FRAME][0]
                    .this_mv.as_mv.col;
            int16_t to_inject_mv_y_l0 =
                context_ptr->md_local_cu_unit[context_ptr->blk_geom->blkidx_mds]
                    .ed_ref_mv_stack[LAST_BWD_FRAME][0]
        ...
        if (context_ptr->near_mv_injection) {
            ...
            // NEAR_NEAR
            if (allow_bipred) {
                maxDrlIndex = GetMaxDrlIndex(xd->ref_mv_count[LAST_BWD_FRAME],
                                             NEAR_NEARMV);
                // maxDrlIndex = 1;
                for (drli = 0; drli < maxDrlIndex; drli++) {
                    get_av1_mv_pred_drl(context_ptr,
...
void inject_new_nearest_new_comb_candidates(
    ...
    PictureControlSet *picture_control_set_ptr, MvReferenceFrame ref_pair,
    uint32_t *candTotCnt) {
    uint8_t inj_mv;
    uint32_t canIdx = *candTotCnt;
    ModeDecisionCandidate *candidateArray = context_ptr->fast_candidate_array;
        ...
        if (rf[1] != NONE_FRAME) {
            {
                ...
                int16_t to_inject_mv_x_l0 =
                    context_ptr
                        ->md_local_cu_unit[context_ptr->blk_geom->blkidx_mds]
                        .ed_ref_mv_stack[ref_pair][0]
                        .this_mv.as_mv.col;
                int16_t to_inject_mv_y_l0 =
                    context_ptr
                ...
                if (inj_mv) {
                    ...
                    candidateArray[canIdx].motion_vector_pred_x[REF_LIST_0] =
                        bestPredmv[0].as_mv.col;
                    candidateArray[canIdx].motion_vector_pred_y[REF_LIST_0] =
                        bestPredmv[0].as_mv.row;
                    candidateArray[canIdx].motion_vector_pred_x[REF_LIST_1] =
                        bestPredmv[1].as_mv.col;
                    candidateArray[canIdx].motion_vector_pred_y[REF_LIST_1] =
                        bestPredmv[1].as_mv.row;
...

            {
                ...
                int16_t to_inject_mv_x_l0 =
                    me_results
                        ->me_mv_array[context_ptr->me_block_offset][ref_idx_0]
                        .x_mv
                    << 1;
                int16_t to_inject_mv_y_l0 =
                    me_results
...
void inject_warped_motion_candidates(PictureControlSet *picture_control_set_ptr,
                                     struct ModeDecisionContext *context_ptr,
                                     CodingUnit *cu_ptr, uint32_t *candTotCnt,
    ...
    candidateArray[canIdx].transform_type[PLANE_TYPE_UV] = DCT_DCT;
        ...
        MvUnit mv_unit;
        mv_unit.mv[0] = mv_0;
        candidateArray[canIdx].local_warp_valid =
            warped_motion_parameters(picture_control_set_ptr,
                                     context_ptr->cu_ptr,
                                     &mv_unit,
...
#if MEMORY_FOOTPRINT_OPT_ME_MV
    uint8_t total_me_cnt =
        meResult->total_me_candidate_index[context_ptr->me_block_offset];
    const MeCandidate *me_block_results =
        meResult->me_candidate[context_ptr->me_block_offset];
#else
    ...
    for (uint8_t me_candidate_index = 0; me_candidate_index < total_me_cnt;
                ...
                int16_t to_inject_mv_x =
                    use_close_loop_me
                        ? ss_mecontext
                              ...
                              << 1
                        : mePuResult->x_mv_l0
                              << 1;  // context_ptr->cu_ptr->ref_mvs[LAST_FRAME][0].as_mv.col;
                int16_t to_inject_mv_y =
                    use_close_loop_me
                        ? ss_mecontext
                                  ->inloop_me_mv[0][0][close_loop_me_index][1]
                              << 1
                        : mePuResult->y_mv_l0
                              << 1;  // context_ptr->cu_ptr->ref_mvs[LAST_FRAME][0].as_mv.row;
                to_inject_mv_x += neighbors[i].col;
                to_inject_mv_y += neighbors[i].row;
                if (context_ptr->injected_mv_count_l0 == 0 ||
                    is_already_injected_mv_l0(context_ptr,
                                              to_inject_mv_x,
                                              to_inject_mv_y) == EB_FALSE) {
#endif
        ...
        candidateArray[canIdx].motion_vector_xl0 =
            use_close_loop_me
                ? ss_mecontext->inloop_me_mv[0][0][close_loop_me_index][0] << 1
                : meResult->me_mv_array[context_ptr->me_block_offset]
                                       [list0_ref_index]
                                           .x_mv
                      << 1;
        candidateArray[canIdx].motion_vector_yl0 =
            use_close_loop_me
                ? ss_mecontext->inloop_me_mv[0][0][close_loop_me_index][1] << 1
                : meResult->me_mv_array[context_ptr->me_block_offset]
                                       [list0_ref_index]
                                           .y_mv
                      << 1;
...
void inject_new_candidates(const SequenceControlSet *sequence_control_set_ptr,
                           struct ModeDecisionContext *context_ptr,
                           PictureControlSet *picture_control_set_ptr,
                           EbBool isCompoundEnabled, EbBool allow_bipred,
                           uint32_t me_sb_addr, SsMeContext *inloop_me_context,
                           EbBool use_close_loop_me,
                           uint32_t close_loop_me_index,
    ...
    const MeLcuResults *me_results =
        picture_control_set_ptr->parent_pcs_ptr->me_results[me_sb_addr];
    uint8_t total_me_cnt =
        me_results->total_me_candidate_index[me_block_offset];
    const MeCandidate *me_block_results =
        me_results->me_candidate[me_block_offset];
        ...
        ************* */
        if (inter_direction == 0) {
            ...
            int16_t to_inject_mv_x =
                use_close_loop_me
                    ? inloop_me_context
                              ->inloop_me_mv[0][0][close_loop_me_index][0]
                          << 1
                    : me_results->me_mv_array[me_block_offset][list0_ref_index]
                              .x_mv
                          << 1;
            int16_t to_inject_mv_y =
                use_close_loop_me
            ...
            if (context_ptr->injected_mv_count_l0 == 0 ||
                ...
                candidateArray[canTotalCnt].motion_vector_pred_x[REF_LIST_0] =
                    bestPredmv[0].as_mv.col;
                candidateArray[canTotalCnt].motion_vector_pred_y[REF_LIST_0] =
                    bestPredmv[0].as_mv.row;
...
#if MRP_DUPLICATION_FIX
                context_ptr->injected_ref_type_l0_array
                    [context_ptr->injected_mv_count_l0] = to_inject_ref_type;
#endif
                ++context_ptr->injected_mv_count_l0;
            }
        }
...
#if MD_INJECTION
void inject_inter_candidates(PictureControlSet *picture_control_set_ptr,
    ...
    if (sequence_control_set_ptr->sb_size == BLOCK_128X128) {
        ...
        uint32_t me_pic_width_in_sb = (sequence_control_set_ptr->luma_width +
                                       sequence_control_set_ptr->sb_sz - 1) /
                                      me_sb_size;
    ...
    context_ptr->me_block_offset =
        (context_ptr->blk_geom->bwidth == 4 ||
         context_ptr->blk_geom->bheight == 4 ||
         context_ptr->blk_geom->bwidth == 128 ||
         context_ptr->blk_geom->bheight == 128)
            ? 0
            : get_me_info_index(max_number_of_pus_per_sb,
                                context_ptr->blk_geom,
                                geom_offset_x,
                                geom_offset_y);
#else
    ...
    me2Nx2NTableOffset = (context_ptr->blk_geom->bwidth == 4 ||
                          context_ptr->blk_geom->bheight == 4 ||
                          context_ptr->blk_geom->bwidth == 128 ||
                          context_ptr->blk_geom->bheight == 128)
                             ? 0
...
#if MD_INJECTION
#if NEW_NEAREST_NEW_INJECTION
    MeLcuResults *me_results = picture_control_set_ptr->parent_pcs_ptr
                                   ->me_results[context_ptr->me_sb_addr];
#else
    ...
    EbBool use_close_loop_me =
        picture_control_set_ptr->parent_pcs_ptr
                    ->enable_in_loop_motion_estimation_flag &&
                ((context_ptr->blk_geom->bwidth == 4 ||
                  context_ptr->blk_geom->bheight == 4) ||
                 (context_ptr->blk_geom->bwidth > 64 ||
                  context_ptr->blk_geom->bheight > 64))
            ? EB_TRUE
            : EB_FALSE;
    ...
    uint32_t close_loop_me_index =
        use_close_loop_me
            ? get_in_loop_me_info_index(
                  MAX_SS_ME_PU_COUNT,
                  sequence_control_set_ptr->sb_size == BLOCK_128X128 ? 1 : 0,
                  context_ptr->blk_geom)
            : 0;
#if BASE_LAYER_REF || MRP_REF_MODE
    ...
    if (picture_control_set_ptr->parent_pcs_ptr->nsq_search_level >=
            NSQ_SEARCH_LEVEL1 &&
        picture_control_set_ptr->parent_pcs_ptr->nsq_search_level <
            NSQ_SEARCH_FULL) {
        inject_newmv_candidate =
    ...
    av1_count_overlappable_neighbors(picture_control_set_ptr,
                                     context_ptr->cu_ptr,
    ...
    ************* */
...
#if INJ_MVP
    uint32_t refIt;
    // all of ref pairs: (1)single-ref List0  (2)single-ref List1  (3)compound
    // Bi-Dir List0-List1  (4)compound Uni-Dir List0-List0  (5)compound Uni-Dir
    // List1-List1
    for (refIt = 0;
         refIt < picture_control_set_ptr->parent_pcs_ptr->tot_ref_frame_types;
         ++refIt) {
        MvReferenceFrame ref_frame_pair =
            picture_control_set_ptr->parent_pcs_ptr->ref_frame_type_arr[refIt];
        inject_mvp_candidates_II(context_ptr,
                                 picture_control_set_ptr,
                                 context_ptr->cu_ptr,
                                 ref_frame_pair,
                                 &canTotalCnt);
    ...
    InjectAv1MvpCandidates(context_ptr,
                           context_ptr->cu_ptr,
                           refFrames,
                           picture_control_set_ptr,
                           lcuAddr,
    ...
    if (context_ptr->new_nearest_near_comb_injection) {
        ...
        if (allow_compound) {
            ...
            // (3)compound Bi-Dir List0-List1  (4)compound Uni-Dir List0-List0
            // (5)compound Uni-Dir List1-List1
            for (refIt = 0;
                 refIt <
                 picture_control_set_ptr->parent_pcs_ptr->tot_ref_frame_types;
                 ++refIt) {
                MvReferenceFrame ref_frame_pair =
                    picture_control_set_ptr->parent_pcs_ptr
                        ->ref_frame_type_arr[refIt];
                inject_new_nearest_new_comb_candidates(sequence_control_set_ptr,
                                                       context_ptr,
                                                       picture_control_set_ptr,
                                                       ref_frame_pair,
                                                       &canTotalCnt);
    ...
    if (inject_newmv_candidate) {
#if ENHANCED_Nx4_4xN_NEW_MV
        inject_new_candidates(sequence_control_set_ptr,
                              context_ptr,
                              picture_control_set_ptr,
                              isCompoundEnabled,
                              allow_bipred,
        ...
        if (context_ptr->nx4_4xn_parent_mv_injection) {
            ...
            uint32_t bwidth_offset_to_8 = (context_ptr->blk_geom->bwidth == 4)
                                          << 2;
            uint32_t bheight_offset_to_8 = (context_ptr->blk_geom->bheight == 4)
                                           << 2;

            // if there is an offset needed to set either dimension to 8
            if (bwidth_offset_to_8 || bheight_offset_to_8) {
                // Align parent block has dimensions inherited by current block,
                // if current block has a dimension of 4 add 4 so the resulting
                // block follows an 8x8 basis
                uint32_t bwidth_to_search =
                    context_ptr->blk_geom->bwidth + bwidth_offset_to_8;
                uint32_t bheight_to_search =
                    context_ptr->blk_geom->bheight + bheight_offset_to_8;

                // Align parent block has origin inherited by current block
                uint32_t x_to_search =
                    context_ptr->blk_geom->origin_x -
                    (geom_offset_x +
                     ((context_ptr->blk_geom->origin_x & 0x7) ? 4 : 0));
                uint32_t y_to_search =
                    context_ptr->blk_geom->origin_y -
                    (geom_offset_y +
                     ((context_ptr->blk_geom->origin_y & 0x7) ? 4 : 0));

                // Search the me_info_index of the parent block
                uint32_t me_info_index;
                for (uint32_t block_index = 0;
                     block_index < max_number_of_pus_per_sb;
                     block_index++) {
            ...
            if (isCompoundEnabled) {
               ...
               ************* */
                if (inter_direction == 1) {
                    ...
                    int16_t to_inject_mv_x =
                        use_close_loop_me
                            ? ss_mecontext->inloop_me_mv[1][0]
                                                        [close_loop_me_index][0]
                                  << 1
                            : me_results
                                      ->me_mv_array[context_ptr
                                                        ->me_block_offset]
                ...
                ************* */
                if (allow_bipred) {
                    if (inter_direction == 2) {
                        ...
                        uint8_t to_inject_ref_type = av1_ref_frame_type(rf);
                        if (context_ptr->injected_mv_count_bipred == 0 ||
                            mrp_is_already_injected_mv_bipred(
                                context_ptr,
                                to_inject_mv_x_l0,
                                to_inject_mv_y_l0,
                                to_inject_mv_x_l1,
                        ...
                        if (context_ptr->injected_mv_count_bipred == 0 ||
                            ...
                            candidateArray[canTotalCnt].is_compound = 1;
                            candidateArray[canTotalCnt]
                                .prediction_direction[0] = (EbPredDirection)2;
#if MRP_LIST_REF_IDX_TYPE_LT
                            MvReferenceFrame rf[2];
#if MRP_MD_UNI_DIR_BIPRED
                            rf[0] = svt_get_ref_frame_type(
                            ...
                            rf[1] = svt_get_ref_frame_type(
                                ...
                                list1_ref_index);
#else
                            rf[0] = svt_get_ref_frame_type(REF_LIST_0,
                                                           list0_ref_index);
                            rf[1] = svt_get_ref_frame_type(REF_LIST_1,
                                                           list1_ref_index);
#endif
                            candidateArray[canTotalCnt].ref_frame_type =
                                av1_ref_frame_type(rf);
...
#if ATB_TX_TYPE_SUPPORT_PER_TU
                            candidateArray[canTotalCnt].transform_type[0] =
                                DCT_DCT;
    ...
    if (context_ptr->global_mv_injection) {
        ...
        ************* */
        {
            int16_t to_inject_mv_x =
                (int16_t)(picture_control_set_ptr->parent_pcs_ptr
                              ->global_motion[LAST_FRAME]
                              .wmmat[1] >>
    ...
    if (picture_control_set_ptr->parent_pcs_ptr->allow_warped_motion &&
        ...
        inject_warped_motion_candidates(picture_control_set_ptr,
                                        context_ptr,
                                        context_ptr->cu_ptr,
                                        &canTotalCnt,
                                        ss_mecontext,
    ...
    if (inject_newmv_candidate &&
        ...
        if (allow_bipred) {
                ...
                if (context_ptr->bipred3x3_injection > 0)
                    if (picture_control_set_ptr->slice_type == B_SLICE)
                        eighth_pel_bipred_refinement(sequence_control_set_ptr,
                                                     picture_control_set_ptr,
                                                     context_ptr,
                                                     me_sb_addr,
                                                     ss_mecontext,
            ...
            if (context_ptr->unipred3x3_injection > 0)
                if (picture_control_set_ptr->slice_type != I_SLICE)
                    eighth_pel_unipred_refinement(sequence_control_set_ptr,
                                                  picture_control_set_ptr,
                                                  context_ptr,
                                                  me_sb_addr,
                                                  ss_mecontext,
    ...
    void inject_inter_candidates(
        ...
        static MvReferenceFrame ref_frames[] = {
            LAST_FRAME, BWDREF_FRAME, LAST_BWD_FRAME};
        EbBool isCompoundEnabled =
            (picture_control_set_ptr->parent_pcs_ptr->reference_mode ==
             SINGLE_REFERENCE)
                ? 0
        ...
        uint32_t close_loop_me_index =
            use_close_loop_me
                ? get_in_loop_me_info_index(
                      MAX_SS_ME_PU_COUNT,
                      sequence_control_set_ptr->sb_size == BLOCK_128X128 ? 1
                                                                         : 0,
                      context_ptr->blk_geom)
                : 0;
#if BASE_LAYER_REF || MRP_REF_MODE
        ...
        ************* */
        ...
        if (inject_newmv_candidate) {
            ...
            for (uint8_t total_me_candidate_index = 0;
                 total_me_candidate_index <
                 mePuResult->total_me_candidate_index;
                 total_me_candidate_index++) {
                switch (
                    mePuResult->distortion_direction[total_me_candidate_index]
                        .direction) {
                case UNI_PRED_LIST_0:
                    distortion_l0 =
                        mePuResult
                            ->distortion_direction[total_me_candidate_index]
                            .distortion;
                    break;
                case UNI_PRED_LIST_1:
                    distortion_l1 =
           ...
           ************* */
            int16_t to_inject_mv_x =
                use_close_loop_me
                    ? ss_mecontext->inloop_me_mv[0][0][close_loop_me_index][0]
                          << 1
                    : mePuResult->x_mv_l0 << 1;
            int16_t to_inject_mv_y =
                use_close_loop_me
                    ? ss_mecontext->inloop_me_mv[0][0][close_loop_me_index][1]
                          << 1
                    : mePuResult->y_mv_l0 << 1;
            if (context_ptr->injected_mv_count_l0 == 0 ||
                is_already_injected_mv_l0(
                    context_ptr, to_inject_mv_x, to_inject_mv_y) == EB_FALSE) {
                candidateArray[canTotalCnt].type = INTER_MODE;
                candidateArray[canTotalCnt].distortion_ready =
                    context_ptr->inter_fast_loop_src_src ? 1 : 0;
                ...
                ************* */
                if (allow_bipred) {
                    ...
                    if (context_ptr->injected_mv_count_bipred == 0 ||
                        ...
                        candidateArray[canTotalCnt].distortion_ready =
                            context_ptr->inter_fast_loop_src_src ? 1 : 0;
                        candidateArray[canTotalCnt].me_distortion =
                            distortion_bipred;
                        candidateArray[canTotalCnt].use_intrabc = 0;
                        candidateArray[canTotalCnt].merge_flag = EB_FALSE;
        ...
        if (inject_newmv_candidate) {
            ...
            if (allow_bipred) {
                ...
                if (context_ptr->unipred3x3_injection > 0)
                    if (picture_control_set_ptr->slice_type != I_SLICE)
                        Unipred3x3CandidatesInjection(picture_control_set_ptr,
                                                      context_ptr,
                                                      sb_ptr,
                                                      me_sb_addr,
                                                      ss_mecontext,
...
static INLINE TxType av1_get_tx_type(
    BlockSize sb_type, int32_t is_inter, PredictionMode pred_mode,
    UvPredictionMode pred_mode_uv, PlaneType plane_type, const MacroBlockD *xd,
    int32_t blk_row, int32_t blk_col, TxSize tx_size, int32_t reduced_tx_set) {
    UNUSED(sb_type);
    UNUSED(*xd);
    UNUSED(blk_row);
    UNUSED(blk_col);
    ...
    if (/*xd->lossless[mbmi->segment_id] ||*/ txsize_sqr_up_map[tx_size] >
        TX_32X32)
        tx_type = DCT_DCT;
    else {
        if (plane_type == PLANE_TYPE_Y) {
            ...
            // In intra mode, uv planes don't share the same prediction mode as
            // y plane, so the tx_type should not be shared
...
void inject_intra_candidates_ois(PictureControlSet *picture_control_set_ptr,
                                 ModeDecisionContext *context_ptr,
                                 LargestCodingUnit *sb_ptr,
...
#if CFL_FIX
    EbBool disable_cfl_flag = (MAX(context_ptr->blk_geom->bheight,
    ...
    OisSbResults *ois_sb_results_ptr =
        picture_control_set_ptr->parent_pcs_ptr->ois_sb_results[sb_ptr->index];
    OisCandidate *ois_blk_ptr =
        ois_sb_results_ptr->ois_candidate_array
            [ep_to_pa_block_index[context_ptr->blk_geom->blkidx_mds]];
    uint8_t total_intra_luma_mode =
        ois_sb_results_ptr->total_ois_intra_candidate
            [ep_to_pa_block_index[context_ptr->blk_geom->blkidx_mds]];

    for (intra_candidate_counter = 0;
         intra_candidate_counter < total_intra_luma_mode;
        ...
        if (av1_is_directional_mode((PredictionMode)intra_mode)) {
            ...
            candidate_array[can_total_cnt].me_distortion =
                ois_blk_ptr[can_total_cnt].distortion;
            ...
            candidate_array[can_total_cnt].use_angle_delta =
                use_angle_delta
                    ? candidate_array[can_total_cnt].is_directional_mode_flag
                    : 0;
#endif
            candidate_array[can_total_cnt].angle_delta[PLANE_TYPE_Y] =
                angle_delta;
            candidate_array[can_total_cnt].intra_chroma_mode =
                disable_cfl_flag
                    ? intra_luma_to_chroma[intra_mode]
                    : context_ptr->chroma_level <= CHROMA_MODE_1 ? UV_CFL_PRED
                                                                 : UV_DC_PRED;
            ...
            candidate_array[can_total_cnt].is_directional_chroma_mode_flag =
                (uint8_t)av1_is_directional_mode(
                    (PredictionMode)candidate_array[can_total_cnt]
                        .intra_chroma_mode);
...
#else
                candidate_array[can_total_cnt].transform_type[PLANE_TYPE_Y] =
                    DCT_DCT;

                if (candidate_array[can_total_cnt].intra_chroma_mode ==
                    UV_CFL_PRED)
                    candidate_array[can_total_cnt]
                        .transform_type[PLANE_TYPE_UV] = DCT_DCT;
                else
                    candidate_array[can_total_cnt]
                    ...
                    av1_get_tx_type(
                        ...
                        (PredictionMode)candidate_array[can_total_cnt]
                            .intra_luma_mode,
                        (UvPredictionMode)candidate_array[can_total_cnt]
                            .intra_chroma_mode,
                        PLANE_TYPE_UV,
                        0,
                        0,
            ...
            candidate_array[can_total_cnt].pred_mode =
...
static INLINE void setup_pred_plane(struct Buf2D *dst, BlockSize bsize,
    ...
    dst->buf =
        src + (y * stride + x);  // scaled_buffer_offset(x, y, stride, scale);
    dst->buf0 = src;
    dst->width = width;
    dst->height = height;
    dst->stride = stride;
}
void av1_setup_pred_block(BlockSize sb_type, struct Buf2D dst[MAX_MB_PLANE],
...
void av1_init_me_luts(void) {
    init_me_luts_bd(
        sad_per_bit16lut_8, sad_per_bit4lut_8, QINDEX_RANGE, AOM_BITS_8);
}
...
void intra_bc_search(PictureControlSet *pcs, ModeDecisionContext *context_ptr,
                     const SequenceControlSet *scs, LargestCodingUnit *sb_ptr,
                     CodingUnit *cu_ptr, MV *dv_cand, uint8_t *num_dv_cand) {
    IntraBcContext x_st;
    ...
    x->is_exhaustive_allowed = context_ptr->blk_geom->bwidth == 4 ||
                                   ...
                                   ? 1
                                   : 0;
    // CHKN crc calculator could be moved to mdContext and these init at init
    // time.
    ...
    x->mv_limits.col_min =
        -(((mi_col + mi_width) * MI_SIZE) + AOM_INTERP_EXTEND);
    ...
    for (int xi = 0; xi < 2; xi++)
        for (int yj = 0; yj < 2; yj++)
            x->hash_value_buffer[xi][yj] = (uint32_t *)malloc(
                AOM_BUFFER_SIZE_FOR_BLOCK_HASH * sizeof(uint32_t));
    ...
    link_Eb_to_aom_buffer_desc_8bit(pcs->parent_pcs_ptr->enhanced_picture_ptr,
                                    &cur_buf);
    ...
    enum IntrabcMotionDirection max_dir = pcs->parent_pcs_ptr->ibc_mode > 1
                                              ? IBC_MOTION_LEFT
                                              : IBC_MOTION_DIRECTIONS;
...
#define INT_VAR_MAX 2147483647  // maximum (signed) int value
        ...
        const int bestsme = av1_full_pixel_search(pcs,
                                                  x,
                                                  bsize,
                                                  &mvp_full,
                                                  step_param,
        ...
        if (mv_check_bounds(&x->mv_limits, &dv))
            continue;
        if (!av1_is_dv_valid(dv, xd, mi_row, mi_col, bsize, scs->mib_size_log2))
            continue;
...
void inject_intra_bc_candidates(
    ...
    ModeDecisionContext *context_ptr,
    const SequenceControlSet *sequence_control_set_ptr,
    LargestCodingUnit *sb_ptr, CodingUnit *cu_ptr, uint32_t *cand_cnt) {
    MV dv_cand[2];
    uint8_t num_dv_cand = 0;

    // perform dv-pred + search up to 2 dv(s)
    intra_bc_search(picture_control_set_ptr,
                    context_ptr,
                    sequence_control_set_ptr,
                    sb_ptr,
                    cu_ptr,
    ...
    for (dv_i = 0; dv_i < num_dv_cand; dv_i++) {
        ...
        candidateArray[*cand_cnt].motion_vector_pred_x[REF_LIST_0] =
            context_ptr->md_local_cu_unit[context_ptr->blk_geom->blkidx_mds]
                .ed_ref_mv_stack[INTRA_FRAME][0]
                .this_mv.as_mv.col;
        candidateArray[*cand_cnt].motion_vector_pred_y[REF_LIST_0] =
            context_ptr->md_local_cu_unit[context_ptr->blk_geom->blkidx_mds]
                .ed_ref_mv_stack[INTRA_FRAME][0]
                .this_mv.as_mv.row;
        ...
        candidateArray[*cand_cnt].interp_filters =
...
// END of Function Declarations
void inject_intra_candidates(PictureControlSet *picture_control_set_ptr,
                             ModeDecisionContext *context_ptr,
                             const SequenceControlSet *sequence_control_set_ptr,
                             LargestCodingUnit *sb_ptr,
#if M8_SKIP_BLK
    ...
    if (picture_control_set_ptr->parent_pcs_ptr->intra_pred_mode == 4) {
        ...
        disable_angle_prediction = 1;
#if M9_INTRA
        angleDeltaCandidateCount =
            disable_angle_refinement ? 1 : angleDeltaCandidateCount;
#endif
...
#if INTRA_DC_ONLY
    openLoopIntraCandidate = intra_mode_start;
#else
        for (openLoopIntraCandidate = intra_mode_start;
             openLoopIntraCandidate <= intra_mode_end;
             ++openLoopIntraCandidate)
#endif
    {
        if (av1_is_directional_mode((PredictionMode)openLoopIntraCandidate)) {
            if (!disable_angle_prediction) {
                for (angleDeltaCounter = 0;
                     angleDeltaCounter < angleDeltaCandidateCount;
                     ++angleDeltaCounter) {
#if M9_INTRA
                    int32_t angle_delta =
                    ...
                    int32_t p_angle =
                        mode_to_angle_map[(
                            PredictionMode)openLoopIntraCandidate] +
                        angle_delta * ANGLE_STEP;
                    if (!disable_z2_prediction ||
                        (p_angle <= 90 || p_angle >= 180)) {
                        candidateArray[canTotalCnt].type = INTRA_MODE;
                        candidateArray[canTotalCnt].intra_luma_mode =
                            openLoopIntraCandidate;
                        candidateArray[canTotalCnt].distortion_ready = 0;
                        candidateArray[canTotalCnt].use_intrabc = 0;
                        candidateArray[canTotalCnt].is_directional_mode_flag =
                            (uint8_t)av1_is_directional_mode(
                                (PredictionMode)openLoopIntraCandidate);
#if !SEARCH_UV_CLEAN_UP
                        candidateArray[canTotalCnt].use_angle_delta =
                            use_angle_delta ? candidateArray[canTotalCnt]
                                                  .is_directional_mode_flag
                                            : 0;
#endif
                        candidateArray[canTotalCnt].angle_delta[PLANE_TYPE_Y] =
                            angle_delta;
#if SEARCH_UV_MODE
                        ...
                        if (context_ptr->chroma_level == CHROMA_MODE_0) {
                            ...
                            // Hsan/Omar: why the restriction below ? (i.e.
                            // disable_ang_uv)
                            const int32_t disable_ang_uv =
                                (context_ptr->blk_geom->bwidth == 4 ||
                                 context_ptr->blk_geom->bheight == 4) &&
                                    ...
                                    : 0;
                            candidateArray[canTotalCnt].intra_chroma_mode =
                                disable_cfl_flag ? intra_luma_to_chroma
                                                       [openLoopIntraCandidate]
                        ...
                        candidateArray[canTotalCnt]
                            ...
                            (uint8_t)av1_is_directional_mode(
                                (PredictionMode)candidateArray[canTotalCnt]
                                    .intra_chroma_mode);
                        candidateArray[canTotalCnt].angle_delta[PLANE_TYPE_UV] =
                            0;
#if ATB_TX_TYPE_SUPPORT_PER_TU
                        candidateArray[canTotalCnt].transform_type[0] = DCT_DCT;

                        if (candidateArray[canTotalCnt].intra_chroma_mode ==
                            UV_CFL_PRED)
                            candidateArray[canTotalCnt].transform_type_uv =
                                DCT_DCT;
                        else
            ...
            if (context_ptr->chroma_level == CHROMA_MODE_0) {
                candidateArray[canTotalCnt].intra_chroma_mode =
                    disable_cfl_flag
                        ? context_ptr
                              ->best_uv_mode[openLoopIntraCandidate]
                                            [MAX_ANGLE_DELTA +
                                             candidateArray[canTotalCnt]
                                                 .angle_delta[PLANE_TYPE_Y]]
                        : UV_CFL_PRED;
            ...
            candidateArray[canTotalCnt].cfl_alpha_signs = 0;
            candidateArray[canTotalCnt].cfl_alpha_idx = 0;
            candidateArray[canTotalCnt].is_directional_chroma_mode_flag =
                (uint8_t)av1_is_directional_mode(
                    (PredictionMode)candidateArray[canTotalCnt]
                        .intra_chroma_mode);
            ...
            if (candidateArray[canTotalCnt].intra_chroma_mode == UV_CFL_PRED)
                ...
                if (candidateArray[canTotalCnt].intra_chroma_mode ==
                    ...
                    av1_get_tx_type(
                        ...
                        (PredictionMode)candidateArray[canTotalCnt]
                            .intra_luma_mode,
                        (UvPredictionMode)candidateArray[canTotalCnt]
                            .intra_chroma_mode,
                        PLANE_TYPE_UV,
                        0,
                        0,
...
#if !OPT_LOSSLESS_0
void ProductInitMdCandInjection(ModeDecisionContext *context_ptr,
                                uint32_t *candidateTotalCnt)
...
EbErrorType ProductGenerateMdCandidatesCu(
    LargestCodingUnit *sb_ptr, ModeDecisionContext *context_ptr,
    SsMeContext *ss_mecontext,
#if !M8_SKIP_BLK
    const uint32_t leaf_index,
#endif
    const uint32_t lcuAddr, uint32_t *candidateTotalCountPtr,
    EbPtr interPredContextPtr, PictureControlSet *picture_control_set_ptr) {
    (void)lcuAddr;
    ...
    context_ptr->injected_mv_count_bipred = 0;
#if !OPT_LOSSLESS_0
    ProductInitMdCandInjection(context_ptr, &canTotalCnt);
#endif
    uint8_t sq_index = LOG2F(context_ptr->blk_geom->sq_size) - 2;
...
#if M9_INTRA
        if (picture_control_set_ptr->parent_pcs_ptr->intra_pred_mode >= 5 &&
            context_ptr->blk_geom->sq_size > 4 &&
            context_ptr->blk_geom->shape == PART_N)
#endif
            inject_intra_candidates_ois(
                picture_control_set_ptr, context_ptr, sb_ptr, &canTotalCnt);
        else if (inject_intra_candidate)
            inject_intra_candidates(picture_control_set_ptr,
                                    context_ptr,
                                    sequence_control_set_ptr,
                                    sb_ptr,
#if M8_SKIP_BLK
    ...
    if (picture_control_set_ptr->parent_pcs_ptr->allow_intrabc)
        inject_intra_bc_candidates(picture_control_set_ptr,
                                   context_ptr,
                                   sequence_control_set_ptr,
                                   sb_ptr,
                                   context_ptr->cu_ptr,
    ...
    if (slice_type != I_SLICE) {
        if (inject_inter_candidate)
            inject_inter_candidates(picture_control_set_ptr,
                                    context_ptr,
                                    ss_mecontext,
                                    sequence_control_set_ptr,
                                    sb_ptr,
...
uint8_t product_full_mode_decision(
    struct ModeDecisionContext *context_ptr, CodingUnit *cu_ptr, uint8_t bwidth,
    uint8_t bheight, ModeDecisionCandidateBuffer **buffer_ptr_array,
    uint32_t candidate_total_count, uint8_t *best_candidate_index_array,
    ...
    for (i = 0; i < candidate_total_count; ++i) {
        ...
        // Compute fullCostBis
        if ((*(buffer_ptr_array[candidateIndex]->full_cost_ptr) <
             lowestIntraCost) &&
            buffer_ptr_array[candidateIndex]->candidate_ptr->type ==
                INTRA_MODE) {
            *best_intra_mode =
                buffer_ptr_array[candidateIndex]->candidate_ptr->pred_mode;
            lowestIntraCost =
                *(buffer_ptr_array[candidateIndex]->full_cost_ptr);
        }
    ...
    context_ptr->md_local_cu_unit[cu_ptr->mds_idx].cost =
        *(buffer_ptr_array[lowestCostIndex]->full_cost_ptr);
    context_ptr->md_local_cu_unit[cu_ptr->mds_idx].cost =
        (context_ptr->md_local_cu_unit[cu_ptr->mds_idx].cost -
         buffer_ptr_array[lowestCostIndex]->candidate_ptr->chroma_distortion) +
        buffer_ptr_array[lowestCostIndex]
            ->candidate_ptr->chroma_distortion_inter_depth;

    if (candidate_ptr->type == INTRA_MODE)
        context_ptr->md_local_cu_unit[cu_ptr->mds_idx].cost_luma =
            buffer_ptr_array[lowestCostIndex]->full_cost_luma;

    context_ptr->md_ep_pipe_sb[cu_ptr->mds_idx].merge_cost =
        *buffer_ptr_array[lowestCostIndex]->full_cost_merge_ptr;
    context_ptr->md_ep_pipe_sb[cu_ptr->mds_idx].skip_cost =
        *buffer_ptr_array[lowestCostIndex]->full_cost_skip_ptr;

    if (candidate_ptr->type == INTER_MODE &&
        candidate_ptr->merge_flag == EB_TRUE)
        context_ptr->md_ep_pipe_sb[cu_ptr->mds_idx].chroma_distortion =
            buffer_ptr_array[lowestCostIndex]->candidate_ptr->chroma_distortion;
    context_ptr->md_local_cu_unit[cu_ptr->mds_idx].full_distortion =
        buffer_ptr_array[lowestCostIndex]->candidate_ptr->full_distortion;
    context_ptr->md_local_cu_unit[cu_ptr->mds_idx].chroma_distortion =
        (uint32_t)buffer_ptr_array[lowestCostIndex]
            ->candidate_ptr->chroma_distortion;
    context_ptr->md_local_cu_unit[cu_ptr->mds_idx]
        .chroma_distortion_inter_depth =
        (uint32_t)buffer_ptr_array[lowestCostIndex]
            ->candidate_ptr->chroma_distortion_inter_depth;
...
#endif
    cu_ptr->skip_flag = candidate_ptr->skip_flag;  // note, the skip flag is
                                                   // re-checked in the ENCDEC
                                                   // process
    cu_ptr->block_has_coeff =
        ((candidate_ptr->block_has_coeff) > 0) ? EB_TRUE : EB_FALSE;
#if ATB_DC_CONTEXT_SUPPORT_1
    cu_ptr->quantized_dc[1][0] =
        buffer_ptr_array[lowestCostIndex]->candidate_ptr->quantized_dc[1][0];
    cu_ptr->quantized_dc[2][0] =
        buffer_ptr_array[lowestCostIndex]->candidate_ptr->quantized_dc[2][0];
#else
        cu_ptr->quantized_dc[0] =
            buffer_ptr_array[lowestCostIndex]->candidate_ptr->quantized_dc[0];
        cu_ptr->quantized_dc[1] =
            buffer_ptr_array[lowestCostIndex]->candidate_ptr->quantized_dc[1];
        cu_ptr->quantized_dc[2] =
            buffer_ptr_array[lowestCostIndex]->candidate_ptr->quantized_dc[2];
#endif
    context_ptr->md_local_cu_unit[cu_ptr->mds_idx].count_non_zero_coeffs =
        candidate_ptr->count_non_zero_coeffs;
    ...
    cu_ptr->interp_filters = candidate_ptr->interp_filters;
    {
        ...
        if (cu_ptr->prediction_mode_flag == INTRA_MODE) {
            ...
            pu_ptr->is_directional_mode_flag =
        ...
        pu_ptr->overlappable_neighbors[0] =
            context_ptr->cu_ptr->prediction_unit_array[0]
                .overlappable_neighbors[0];
        pu_ptr->overlappable_neighbors[1] =
            context_ptr->cu_ptr->prediction_unit_array[0]
                .overlappable_neighbors[1];
    ...
    do {
        ...
        txb_ptr->y_has_coeff =
            (EbBool)(((candidate_ptr->y_has_coeff) & (1 << tu_index)) > 0);
        txb_ptr->u_has_coeff =
            (EbBool)(((candidate_ptr->u_has_coeff) & (1 << (tu_index))) > 0);
        txb_ptr->v_has_coeff =
            (EbBool)(((candidate_ptr->v_has_coeff) & (1 << (tu_index))) > 0);
#if ATB_TX_TYPE_SUPPORT_PER_TU
        txb_ptr->transform_type[PLANE_TYPE_Y] =
            candidate_ptr->transform_type[tu_index];
        txb_ptr->transform_type[PLANE_TYPE_UV] =
            candidate_ptr->transform_type_uv;
#else
            txb_ptr->transform_type[PLANE_TYPE_Y] =
                candidate_ptr->transform_type[PLANE_TYPE_Y];
            txb_ptr->transform_type[PLANE_TYPE_UV] =
                candidate_ptr->transform_type[PLANE_TYPE_UV];
#endif

#if ATB_DC_CONTEXT_SUPPORT_1
        cu_ptr->quantized_dc[0][tu_index] =
            candidate_ptr->quantized_dc[0][tu_index];
#endif
        ...
        if (pu_ptr->inter_pred_direction_index == UNI_PRED_LIST_0) {
            cu_ptr->predmv[0].as_mv.col =
                candidate_ptr->motion_vector_pred_x[REF_LIST_0];
            cu_ptr->predmv[0].as_mv.row =
                candidate_ptr->motion_vector_pred_y[REF_LIST_0];
        } else if (pu_ptr->inter_pred_direction_index == UNI_PRED_LIST_1) {
            cu_ptr->predmv[0].as_mv.col =
                candidate_ptr->motion_vector_pred_x[REF_LIST_1];
            cu_ptr->predmv[0].as_mv.row =
                candidate_ptr->motion_vector_pred_y[REF_LIST_1];
        } else if (pu_ptr->inter_pred_direction_index == BI_PRED) {
            cu_ptr->predmv[0].as_mv.col =
                candidate_ptr->motion_vector_pred_x[REF_LIST_0];
            cu_ptr->predmv[0].as_mv.row =
                candidate_ptr->motion_vector_pred_y[REF_LIST_0];
            cu_ptr->predmv[1].as_mv.col =
                candidate_ptr->motion_vector_pred_x[REF_LIST_1];
            cu_ptr->predmv[1].as_mv.row =
                candidate_ptr->motion_vector_pred_y[REF_LIST_1];
        }
#endif
#if NO_ENCDEC
        // copy coeff
        {
            ...
            int32_t *src_ptr =
                &(((int32_t *)buffer_ptr_array[lowestCostIndex]
                       ->residual_quant_coeff_ptr->buffer_y)[txb_1d_offset]);
            int32_t *dst_ptr = &(((int32_t *)context_ptr->cu_ptr->coeff_tmp
            ...
            if (context_ptr->blk_geom->has_uv) {
                ...
                src_ptr = &(((int32_t *)buffer_ptr_array[lowestCostIndex]
                                 ->residual_quant_coeff_ptr
                                 ->buffer_cb)[txb_1d_offset_uv]);
                dst_ptr = &(((int32_t *)context_ptr->cu_ptr->coeff_tmp
                                 ->buffer_cb)[txb_1d_offset_uv]);
