...
void av1_cdef_search(EncDecContext *context_ptr,
                     SequenceControlSet *sequence_control_set_ptr,
                     PictureControlSet *picture_control_set_ptr);

void av1_cdef_frame(EncDecContext *context_ptr,
                    SequenceControlSet *sequence_control_set_ptr,
                    PictureControlSet *pCs);

void av1_cdef_search16bit(EncDecContext *context_ptr,
                          SequenceControlSet *sequence_control_set_ptr,
                          PictureControlSet *picture_control_set_ptr);
void av1_cdef_frame16bit(uint8_t is16bit,
                         SequenceControlSet *sequence_control_set_ptr,
                         PictureControlSet *pCs);

void av1_add_film_grain(EbPictureBufferDesc *src, EbPictureBufferDesc *dst,
                        aom_film_grain_t *film_grain_ptr);

void av1_loop_restoration_save_boundary_lines(const Yv12BufferConfig *frame,
                                              Av1Common *cm,
                                              int32_t after_cdef);
void av1_pick_filter_restoration(
    const Yv12BufferConfig *src,
    Yv12BufferConfig *trial_frame_rst /*Av1Comp *cpi*/, Macroblock *x,
    Av1Common *const cm);
void av1_loop_restoration_filter_frame(Yv12BufferConfig *frame, Av1Common *cm,
                                       int32_t optimized_lr);

const int16_t encMinDeltaQpWeightTab[MAX_TEMPORAL_LAYERS] = {
    100, 100, 100, 100, 100, 100};
const int16_t encMaxDeltaQpWeightTab[MAX_TEMPORAL_LAYERS] = {
    100, 100, 100, 100, 100, 100};
...
const int8_t encMaxDeltaQpTab[4][MAX_TEMPORAL_LAYERS] = {{4, 5, 5, 5, 5, 5},
                                                         {4, 5, 5, 5, 5, 5},
                                                         {4, 5, 5, 5, 5, 5},
                                                         {4, 5, 5, 5, 5, 5}};
...
EbErrorType enc_dec_context_ctor(
    ...
    EbFifo *picture_demux_fifo_ptr, EbBool is16bit, EbColorFormat color_format,
    uint32_t max_input_luma_width, uint32_t max_input_luma_height) {
    ...
    *context_dbl_ptr = context_ptr;
    ...
    EB_MALLOC(int16_t *,
              ...
              3152,
              EB_N_PTR);  // refer to EbInvTransform_SSE2.as. case 32x32
    // MD rate Estimation tables
    EB_MALLOC(MdRateEstimationContext *,
              context_ptr->md_rate_estimation_ptr,
              sizeof(MdRateEstimationContext),
              EB_N_PTR);
    ...
    // Scratch Coeff Buffer
    {
        ...
        return_error = eb_picture_buffer_desc_ctor(
            (EbPtr *)&context_ptr->inverse_quant_buffer, (EbPtr)&init32BitData);

        if (return_error == EB_ErrorInsufficientResources)
            return EB_ErrorInsufficientResources;
        return_error = eb_picture_buffer_desc_ctor(
            (EbPtr *)&context_ptr->transform_buffer, (EbPtr)&init32BitData);
        if (return_error == EB_ErrorInsufficientResources)
            return EB_ErrorInsufficientResources;
        return_error = eb_picture_buffer_desc_ctor(
            (EbPtr *)&context_ptr->residual_buffer, (EbPtr)&initData);
        if (return_error == EB_ErrorInsufficientResources)
            return EB_ErrorInsufficientResources;
    }
    ...
    if (is16bit) {
        return_error =
            intra_reference16bit_samples_ctor(&context_ptr->intra_ref_ptr16);
        if (return_error == EB_ErrorInsufficientResources)
            return EB_ErrorInsufficientResources;
    }
    // Mode Decision Context
    return_error = mode_decision_context_ctor(
        &context_ptr->md_context, color_format, 0, 0);
 ...
 *************************************************/
static void ResetEncodePassNeighborArrays(
    PictureControlSet *picture_control_set_ptr) {
    neighbor_array_unit_reset(
        picture_control_set_ptr->ep_intra_luma_mode_neighbor_array);
    neighbor_array_unit_reset(
        picture_control_set_ptr->ep_intra_chroma_mode_neighbor_array);
    neighbor_array_unit_reset(picture_control_set_ptr->ep_mv_neighbor_array);
    neighbor_array_unit_reset(
        picture_control_set_ptr->ep_skip_flag_neighbor_array);
    neighbor_array_unit_reset(
        picture_control_set_ptr->ep_mode_type_neighbor_array);
    neighbor_array_unit_reset(
        picture_control_set_ptr->ep_leaf_depth_neighbor_array);
    neighbor_array_unit_reset(
        picture_control_set_ptr->ep_luma_recon_neighbor_array);
    neighbor_array_unit_reset(
        picture_control_set_ptr->ep_cb_recon_neighbor_array);
    neighbor_array_unit_reset(
        picture_control_set_ptr->ep_cr_recon_neighbor_array);
...
#if !OPT_LOSSLESS_0
    neighbor_array_unit_reset(
        picture_control_set_ptr->amvp_mv_merge_mv_neighbor_array);
    neighbor_array_unit_reset(
        picture_control_set_ptr->amvp_mv_merge_mode_type_neighbor_array);
#endif
    return;
}
 ...
 * Reset Coding Loop
 **************************************************/
static void ResetEncDec(EncDecContext *context_ptr,
                        PictureControlSet *picture_control_set_ptr,
                        SequenceControlSet *sequence_control_set_ptr,
                        uint32_t segment_index) {
    EB_SLICE slice_type;
    MdRateEstimationContext *md_rate_estimation_array;
    context_ptr->is16bit = (EbBool)(
        sequence_control_set_ptr->static_config.encoder_bit_depth > EB_8BIT);

    // QP
    // context_ptr->qp          =
    // picture_control_set_ptr->parent_pcs_ptr->tilePtrArray[tileIndex]->tileQp;
...
#endif
    // Asuming cb and cr offset to be the same for chroma QP in both slice and
    // pps for lambda computation

    context_ptr->chroma_qp = context_ptr->qp;

    // Lambda Assignement
    context_ptr->qp_index =
        (uint8_t)picture_control_set_ptr->parent_pcs_ptr->base_qindex;
    (*av1_lambda_assignment_function_table
         [picture_control_set_ptr->parent_pcs_ptr->pred_structure])(
        &context_ptr->fast_lambda,
        &context_ptr->full_lambda,
        &context_ptr->fast_chroma_lambda,
        &context_ptr->full_chroma_lambda,
        (uint8_t)picture_control_set_ptr->parent_pcs_ptr->enhanced_picture_ptr
            ->bit_depth,
        context_ptr->qp_index);

    // Slice Type
    slice_type = (picture_control_set_ptr->parent_pcs_ptr->idr_flag == EB_TRUE)
                     ? I_SLICE
                     : picture_control_set_ptr->slice_type;

    // Increment the MD Rate Estimation array pointer to point to the right
    // address based on the QP and slice type
    md_rate_estimation_array =
        (MdRateEstimationContext *)sequence_control_set_ptr->encode_context_ptr
            ->md_rate_estimation_array;
#if ADD_DELTA_QP_SUPPORT
    md_rate_estimation_array +=
        slice_type * TOTAL_NUMBER_OF_QP_VALUES +
        picture_control_set_ptr->parent_pcs_ptr->picture_qp;
#else
    md_rate_estimation_array +=
        slice_type * TOTAL_NUMBER_OF_QP_VALUES + context_ptr->qp;
#endif

    // Reset MD rate Estimation table to initial values by copying from
    // md_rate_estimation_array
    ...
    if (picture_control_set_ptr->parent_pcs_ptr->is_used_as_reference_flag ==
        EB_TRUE)
        context_ptr->reference_object_write_ptr =
            (EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
 ...
 ******************************************************/
static void EncDecConfigureLcu(EncDecContext *context_ptr,
                               LargestCodingUnit *sb_ptr,
                               PictureControlSet *picture_control_set_ptr,
                               SequenceControlSet *sequence_control_set_ptr,
                               uint8_t picture_qp, uint8_t sb_qp) {
    // RC is off
    if (sequence_control_set_ptr->static_config.rate_control_mode == 0 &&
        sequence_control_set_ptr->static_config.improve_sharpness == 0)
        context_ptr->qp = picture_qp;
    // RC is on
    else
    ...
    // pps for lambda computation
    context_ptr->chroma_qp = context_ptr->qp;
    /* Note(CHKN) : when Qp modulation varies QP on a sub-LCU(CU) basis,  Lamda
     * has to change based on Cu->QP , and then this code has to move inside the
     * CU loop in MD */
    (void)sb_ptr;
    context_ptr->qp_index =
        (uint8_t)picture_control_set_ptr->parent_pcs_ptr->base_qindex;
 ...
 ******************************************************/
EbBool AssignEncDecSegments(EncDecSegments *segmentPtr,
                            uint16_t *segmentInOutIndex, EncDecTasks *taskPtr,
                            EbFifo *srmFifoPtr) {
    ...
    case ENCDEC_TASKS_MDC_INPUT:
        ...
        *segmentInOutIndex = segmentPtr->row_array[0].current_seg_index;
        ...
        //    taskPtr->picture_control_set_wrapper_ptr->object_ptr)->picture_number,
        //    *segmentInOutIndex);
    ...
    case ENCDEC_TASKS_ENCDEC_INPUT:
        ...
        *segmentInOutIndex = segmentPtr->row_array[taskPtr->enc_dec_segment_row]
                                 .current_seg_index;
        taskPtr->input_type = ENCDEC_TASKS_CONTINUE;
    ...
    case ENCDEC_TASKS_CONTINUE:
        ...
        // Right Neighbor
        if (segment_index <
            segmentPtr->row_array[rowSegmentIndex].ending_seg_index) {
            eb_block_on_mutex(
                segmentPtr->row_array[rowSegmentIndex].assignment_mutex);
        ...
        if (rowSegmentIndex < segmentPtr->segment_row_count - 1 &&
            bottomLeftSegmentIndex >=
                segmentPtr->row_array[rowSegmentIndex + 1].starting_seg_index) {
            eb_block_on_mutex(
                segmentPtr->row_array[rowSegmentIndex + 1].assignment_mutex);

            --segmentPtr->dep_map.dependency_map[bottomLeftSegmentIndex];

            if (segmentPtr->dep_map.dependency_map[bottomLeftSegmentIndex] ==
                0) {
                if (selfAssigned == EB_TRUE)
                    feedbackRowIndex = (int16_t)rowSegmentIndex + 1;
                else {
                    *segmentInOutIndex =
                        segmentPtr->row_array[rowSegmentIndex + 1]
                            .current_seg_index;
                    ++segmentPtr->row_array[rowSegmentIndex + 1]
                          .current_seg_index;
                    selfAssigned = EB_TRUE;
        ...
        if (feedbackRowIndex > 0) {
            ...
            feedbackTaskPtr->picture_control_set_wrapper_ptr =
                taskPtr->picture_control_set_wrapper_ptr;
...
void ReconOutput(PictureControlSet *picture_control_set_ptr,
                 SequenceControlSet *sequence_control_set_ptr) {
    ...
    if (!picture_control_set_ptr->parent_pcs_ptr->is_alt_ref) {
        ...
        outputReconPtr =
            (EbBufferHeaderType *)outputReconWrapperPtr->object_ptr;
        ...
        // Copy the Reconstructed Picture to the Output Recon Buffer
        {
            ...
            {
                if (picture_control_set_ptr->parent_pcs_ptr
            ...
            // FGN: Create a buffer if needed, copy the reconstructed picture
            // and run the film grain synthesis algorithm
            ...
            if (sequence_control_set_ptr->film_grain_params_present) {
                    ...
                    if (is16bit)
                        intermediateBufferPtr =
                            picture_control_set_ptr
                                ->film_grain_picture16bit_ptr;
                    else
                        intermediateBufferPtr =
                            picture_control_set_ptr->film_grain_picture_ptr;
                }
                ...
                if (picture_control_set_ptr->parent_pcs_ptr
                    ...
                    film_grain_ptr = &picture_control_set_ptr->parent_pcs_ptr
                                          ->film_grain_params;

                av1_add_film_grain(
                    recon_ptr, intermediateBufferPtr, film_grain_ptr);
            ...
            sampleTotalCount =
                ((recon_ptr->max_width -
                  sequence_control_set_ptr->max_input_pad_right) *
                 (recon_ptr->max_height -
                  sequence_control_set_ptr->max_input_pad_bottom))
                << is16bit;
            reconReadPtr =
                recon_ptr->buffer_y +
                (recon_ptr->origin_y << is16bit) * recon_ptr->stride_y +
                (recon_ptr->origin_x << is16bit);
            reconWritePtr =
                &(outputReconPtr->p_buffer[outputReconPtr->n_filled_len]);

            CHECK_REPORT_ERROR(
                (outputReconPtr->n_filled_len + sampleTotalCount <=
                 outputReconPtr->n_alloc_len),
            ...
            reconReadPtr =
                recon_ptr->buffer_cb +
                ((recon_ptr->origin_y << is16bit) >> 1) * recon_ptr->stride_cb +
                ((recon_ptr->origin_x << is16bit) >> 1);
            reconWritePtr =
                &(outputReconPtr->p_buffer[outputReconPtr->n_filled_len]);
            ...
            reconReadPtr =
                recon_ptr->buffer_cr +
                ((recon_ptr->origin_y << is16bit) >> 1) * recon_ptr->stride_cr +
                ((recon_ptr->origin_x << is16bit) >> 1);
            reconWritePtr =
                &(outputReconPtr->p_buffer[outputReconPtr->n_filled_len]);
        ...
        // Overlay and altref have 1 recon only, which is from overlay pictures.
        // So the recon of the alt_ref is not sent to the application. However,
        // to hanlde the end of sequence properly, total_number_of_recon_frames
        // is increamented
        encode_context_ptr->total_number_of_recon_frames++;
    }
#endif
...
void PsnrCalculations(PictureControlSet *picture_control_set_ptr,
                      SequenceControlSet *sequence_control_set_ptr) {
    EbBool is16bit =
        (sequence_control_set_ptr->static_config.encoder_bit_depth > EB_8BIT);
    ...
    if (!is16bit) {
        ...
        reconCoeffBuffer =
            &((recon_ptr->buffer_y)[recon_ptr->origin_x +
                                    recon_ptr->origin_y * recon_ptr->stride_y]);
        inputBuffer =
        ...
        while (row_index < sequence_control_set_ptr->luma_height) {
            ...
            while (columnIndex < sequence_control_set_ptr->luma_width) {
                residualDistortion +=
                    (int64_t)SQR((int64_t)(inputBuffer[columnIndex]) -
                                 (reconCoeffBuffer[columnIndex]));
                ++columnIndex;
            }
        ...
        reconCoeffBuffer = &((recon_ptr->buffer_cb)[recon_ptr->origin_x / 2 +
                                                    recon_ptr->origin_y / 2 *
                                                        recon_ptr->stride_cb]);
        inputBuffer =
        ...
        reconCoeffBuffer = &((recon_ptr->buffer_cr)[recon_ptr->origin_x / 2 +
                                                    recon_ptr->origin_y / 2 *
                                                        recon_ptr->stride_cr]);
        inputBuffer =
        ...
        picture_control_set_ptr->parent_pcs_ptr->luma_sse =
            (uint32_t)sseTotal[0];
        ...
        if (sequence_control_set_ptr->static_config.ten_bit_format == 1) {
            ...
            const uint32_t chroma_width =
                sequence_control_set_ptr->chroma_width;
            ...
            EbByte inputBufferOrg = &(
                (input_picture_ptr->buffer_y)[input_picture_ptr->origin_x +
                                              input_picture_ptr->origin_y *
                                                  input_picture_ptr->stride_y]);
            uint16_t *reconBufferOrg = (uint16_t *)(&(
                (recon_ptr->buffer_y)[(recon_ptr->origin_x << is16bit) +
                                      (recon_ptr->origin_y << is16bit) *
                                          recon_ptr->stride_y]));
            ;

            EbByte inputBufferOrgU =
                &((input_picture_ptr
                       ->buffer_cb)[input_picture_ptr->origin_x / 2 +
                                    input_picture_ptr->origin_y / 2 *
                                        input_picture_ptr->stride_cb]);
            ;
            uint16_t *reconBufferOrgU = reconCoeffBuffer = (uint16_t *)(&(
                (recon_ptr->buffer_cb)[(recon_ptr->origin_x << is16bit) / 2 +
                                       (recon_ptr->origin_y << is16bit) / 2 *
                                           recon_ptr->stride_cb]));
            ;

            EbByte inputBufferOrgV =
                &((input_picture_ptr
                       ->buffer_cr)[input_picture_ptr->origin_x / 2 +
                                    input_picture_ptr->origin_y / 2 *
                                        input_picture_ptr->stride_cr]);
            ;
            uint16_t *reconBufferOrgV = reconCoeffBuffer = (uint16_t *)(&(
                (recon_ptr->buffer_cr)[(recon_ptr->origin_x << is16bit) / 2 +
                                       (recon_ptr->origin_y << is16bit) / 2 *
                                           recon_ptr->stride_cr]));
            ;
            ...
            for (lcuNumberInHeight = 0; lcuNumberInHeight < pictureHeighInLcu;
                 ++lcuNumberInHeight) {
                for (lcuNumberInWidth = 0;
                     lcuNumberInWidth < picture_width_in_sb;
                     ++lcuNumberInWidth) {
                    uint32_t tbOriginX = lcuNumberInWidth * 64;
                    uint32_t tbOriginY = lcuNumberInHeight * 64;
                    uint32_t sb_width = (luma_width - tbOriginX) < 64
                                            ? (luma_width - tbOriginX)
                                            : 64;
                    uint32_t sb_height = (luma_height - tbOriginY) < 64
                                             ? (luma_height - tbOriginY)
                                             : 64;

                    inputBuffer = inputBufferOrg +
                                  tbOriginY * input_picture_ptr->stride_y +
                                  tbOriginX;
                    inputBufferBitInc = input_picture_ptr->buffer_bit_inc_y +
                                        tbOriginY * luma2BitWidth +
                                        (tbOriginX / 4) * sb_height;
                    reconCoeffBuffer = reconBufferOrg +
                                       tbOriginY * recon_ptr->stride_y +
                                       tbOriginX;
                    ...
                    for (j = 0; j < sb_height; j++) {
                        for (k = 0; k < sb_width / 4; k++) {
                            four2bitPels =
                                inputBufferBitInc[k + j * inn_stride];

                            nBitPixel = (four2bitPels >> 6) & 3;
                            outPixel =
                                inputBuffer[k * 4 + 0 +
                                            j * input_picture_ptr->stride_y]
                                << 2;
                            outPixel = outPixel | nBitPixel;
                            residualDistortion += (int64_t)SQR(
                                (int64_t)outPixel -
                                (int64_t)
                                    reconCoeffBuffer[k * 4 + 0 +
                    ...
                    sb_width = (chroma_width - tbOriginX) < 32
                                   ? (chroma_width - tbOriginX)
                                   : 32;
                    sb_height = (chroma_height - tbOriginY) < 32
                                    ? (chroma_height - tbOriginY)
                                    : 32;

                    inn_stride = sb_width / 4;

                    inputBuffer = inputBufferOrgU +
                                  tbOriginY * input_picture_ptr->stride_cb +
                                  tbOriginX;

                    inputBufferBitInc = input_picture_ptr->buffer_bit_inc_cb +
                                        tbOriginY * chroma2BitWidth +
                                        (tbOriginX / 4) * sb_height;

                    reconCoeffBuffer = reconBufferOrgU +
                                       tbOriginY * recon_ptr->stride_cb +
                                       tbOriginX;
                    ...
                    for (j = 0; j < sb_height; j++) {
                        for (k = 0; k < sb_width / 4; k++) {
                            ...
                            outPixel =
                                inputBuffer[k * 4 + 0 +
                                            j * input_picture_ptr->stride_cb]
                                << 2;
                            outPixel = outPixel | nBitPixel;
                            residualDistortionU += (int64_t)SQR(
                                (int64_t)outPixel -
                                (int64_t)
                                    reconCoeffBuffer[k * 4 + 0 +
                    ...
                    inputBuffer = inputBufferOrgV +
                                  tbOriginY * input_picture_ptr->stride_cr +
                                  tbOriginX;
                    inputBufferBitInc = input_picture_ptr->buffer_bit_inc_cr +
                                        tbOriginY * chroma2BitWidth +
                                        (tbOriginX / 4) * sb_height;
                    reconCoeffBuffer = reconBufferOrgV +
                                       tbOriginY * recon_ptr->stride_cr +
                                       tbOriginX;
                    ...
                    for (j = 0; j < sb_height; j++) {
                        for (k = 0; k < sb_width / 4; k++) {
                            ...
                            outPixel =
                                inputBuffer[k * 4 + 0 +
                                            j * input_picture_ptr->stride_cr]
                                << 2;
                            outPixel = outPixel | nBitPixel;
                            residualDistortionV += (int64_t)SQR(
                                (int64_t)outPixel -
                                (int64_t)
                                    reconCoeffBuffer[k * 4 + 0 +
            ...
            while (row_index < sequence_control_set_ptr->luma_height) {
                ...
                while (columnIndex < sequence_control_set_ptr->luma_width) {
                    residualDistortion += (int64_t)SQR(
                        (int64_t)(
                            (((inputBuffer[columnIndex]) << 2) |
                             ((inputBufferBitInc[columnIndex] >> 6) & 3))) -
                        (reconCoeffBuffer[columnIndex]));
...
void PadRefAndSetFlags(PictureControlSet *picture_control_set_ptr,
                       SequenceControlSet *sequence_control_set_ptr) {
    EbReferenceObject *referenceObject =
        (EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
            ->reference_picture_wrapper_ptr->object_ptr;
    EbPictureBufferDesc *refPicPtr =
        (EbPictureBufferDesc *)referenceObject->reference_picture;
    EbPictureBufferDesc *refPic16BitPtr =
        (EbPictureBufferDesc *)referenceObject->reference_picture16bit;
    EbBool is16bit =
        (sequence_control_set_ptr->static_config.encoder_bit_depth > EB_8BIT);

    if (!is16bit) {
        // Y samples
        generate_padding(refPicPtr->buffer_y,
                         refPicPtr->stride_y,
                         refPicPtr->width,
                         refPicPtr->height,
                         refPicPtr->origin_x,
        ...
        // Cb samples
        generate_padding(refPicPtr->buffer_cb,
                         refPicPtr->stride_cb,
                         refPicPtr->width >> 1,
                         refPicPtr->height >> 1,
        ...
        // Cr samples
        generate_padding(refPicPtr->buffer_cr,
                         refPicPtr->stride_cr,
                         refPicPtr->width >> 1,
                         refPicPtr->height >> 1,
                         refPicPtr->origin_x >> 1,
    ...
    if (is16bit) {
        // Y samples
        generate_padding16_bit(refPic16BitPtr->buffer_y,
        ...
        // Cb samples
        generate_padding16_bit(refPic16BitPtr->buffer_cb,
        ...
        // Cr samples
        generate_padding16_bit(refPic16BitPtr->buffer_cr,
        ...
        un_pack2d((uint16_t *)refPic16BitPtr->buffer_y,
        ...
        un_pack2d((uint16_t *)refPic16BitPtr->buffer_cb,
        ...
        un_pack2d((uint16_t *)refPic16BitPtr->buffer_cr,
...
#else
    referenceObject->qp =
        (uint8_t)picture_control_set_ptr->parent_pcs_ptr->picture_qp;
#endif

    // set up the Slice Type
    referenceObject->slice_type =
        picture_control_set_ptr->parent_pcs_ptr->slice_type;
}

void CopyStatisticsToRefObject(PictureControlSet *picture_control_set_ptr,
                               SequenceControlSet *sequence_control_set_ptr) {
    picture_control_set_ptr->intra_coded_area =
        (100 * picture_control_set_ptr->intra_coded_area) /
    ...
    ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
         ->reference_picture_wrapper_ptr->object_ptr)
        ->intra_coded_area =
        (uint8_t)(picture_control_set_ptr->intra_coded_area);

    uint32_t sb_index;
    for (sb_index = 0; sb_index < picture_control_set_ptr->sb_total_count;
         ++sb_index)
        ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
             ->reference_picture_wrapper_ptr->object_ptr)
    ...
    ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
         ->reference_picture_wrapper_ptr->object_ptr)
        ->penalize_skipflag = EB_FALSE;
    if (picture_control_set_ptr->slice_type == B_SLICE) {
        // MRP_MD
        refObjL0 = (EbReferenceObject *)picture_control_set_ptr
                       ->ref_pic_ptr_array[REF_LIST_0]
                       ->object_ptr;
        refObjL1 = (EbReferenceObject *)picture_control_set_ptr
                       ->ref_pic_ptr_array[REF_LIST_1]
                       ->object_ptr;

        if (picture_control_set_ptr->temporal_layer_index == 0) {
            if (picture_control_set_ptr->parent_pcs_ptr
                    ->intra_coded_block_probability > 30)
                ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
                     ->reference_picture_wrapper_ptr->object_ptr)
                    ->penalize_skipflag = EB_TRUE;
        } else
            ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
                 ->reference_picture_wrapper_ptr->object_ptr)
                ->penalize_skipflag =
                (refObjL0->penalize_skipflag || refObjL1->penalize_skipflag)
                    ? EB_TRUE
                    : EB_FALSE;
    }
#endif
    ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
         ->reference_picture_wrapper_ptr->object_ptr)
        ->tmp_layer_idx =
        (uint8_t)picture_control_set_ptr->temporal_layer_index;
    ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
         ->reference_picture_wrapper_ptr->object_ptr)
        ->is_scene_change =
        picture_control_set_ptr->parent_pcs_ptr->scene_change_flag;

    ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
         ->reference_picture_wrapper_ptr->object_ptr)
        ->cdef_frame_strength =
        picture_control_set_ptr->parent_pcs_ptr->cdef_frame_strength;
...
EbErrorType QpmDeriveWeightsMinAndMax(
    ...
    EbErrorType return_error = EB_ErrorNone;
    uint32_t cu_depth;
    context_ptr->min_delta_qp_weight =
        encMinDeltaQpWeightTab[picture_control_set_ptr->temporal_layer_index];
    context_ptr->max_delta_qp_weight =
        encMaxDeltaQpWeightTab[picture_control_set_ptr->temporal_layer_index];
...

    {
        if (picture_control_set_ptr->slice_type == I_SLICE) {
            if (picture_control_set_ptr->parent_pcs_ptr
                        ->percentage_of_edgein_light_background > 0 &&
                picture_control_set_ptr->parent_pcs_ptr
                        ->percentage_of_edgein_light_background <= 3 &&
                !adjust_min_qp_flag &&
                picture_control_set_ptr->parent_pcs_ptr
                    ->dark_back_groundlight_fore_ground) {
                context_ptr->min_delta_qp_weight = 100;
            } else {
                if (adjust_min_qp_flag)
                    context_ptr->min_delta_qp_weight = 250;
                else if (picture_control_set_ptr->parent_pcs_ptr
                             ->pic_homogenous_over_time_sb_percentage > 30) {
        ...
        if (picture_control_set_ptr->parent_pcs_ptr
                ->high_dark_area_density_flag) {
            context_ptr->min_delta_qp_weight = 25;
            context_ptr->max_delta_qp_weight = 25;
        }
    }

    // Refine max_delta_qp_weight; apply conservative max_degrade_weight when
    // most of the picture is homogenous over time.
    if (picture_control_set_ptr->parent_pcs_ptr
            ->pic_homogenous_over_time_sb_percentage > 90)
        context_ptr->max_delta_qp_weight =
            context_ptr->max_delta_qp_weight >> 1;
    for (cu_depth = 0; cu_depth < 4; cu_depth++) {
        context_ptr->min_delta_qp[cu_depth] =
            picture_control_set_ptr->slice_type == I_SLICE
                ? encMinDeltaQpISliceTab[cu_depth]
                : encMinDeltaQpTab[cu_depth][picture_control_set_ptr
                                                 ->temporal_layer_index];
        context_ptr->max_delta_qp[cu_depth] =
            encMaxDeltaQpTab[cu_depth]
...
EbErrorType signal_derivation_enc_dec_kernel_oq(
    ...
    if (picture_control_set_ptr->parent_pcs_ptr->sc_content_detected)
        ...
        else if (picture_control_set_ptr->parent_pcs_ptr->slice_type == I_SLICE)
    ...
    if (picture_control_set_ptr->enc_mode <= ENC_M7)
        ...
        // Level                Settings
        // 0                    Injection off (Hsan: but not derivation as used
        // by MV ref derivation) 1                    On
#if SCREEN_CONTENT_SETTINGS
    if (picture_control_set_ptr->parent_pcs_ptr->sc_content_detected)
        if (picture_control_set_ptr->enc_mode <= ENC_M1)
...
void move_cu_data(CodingUnit *src_cu, CodingUnit *dst_cu);
...
void *enc_dec_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        eb_get_full_object(context_ptr->mode_decision_input_fifo_ptr,
                           &encDecTasksWrapperPtr);

        encDecTasksPtr = (EncDecTasks *)encDecTasksWrapperPtr->object_ptr;
        picture_control_set_ptr =
            (PictureControlSet *)
                encDecTasksPtr->picture_control_set_wrapper_ptr->object_ptr;
        ...
        is16bit =
            (EbBool)(sequence_control_set_ptr->static_config.encoder_bit_depth >
                     EB_8BIT);
        (void)is16bit;
        (void)endOfRowFlag;
        ...
        signal_derivation_enc_dec_kernel_oq(sequence_control_set_ptr,
                                            picture_control_set_ptr,
        ...
        picture_width_in_sb =
            (sequence_control_set_ptr->luma_width + sb_sz - 1) >> lcuSizeLog2;
        endOfRowFlag = EB_FALSE;
        lcuRowIndexStart = lcuRowIndexCount = 0;
        context_ptr->tot_intra_coded_area = 0;

        // Segment-loop
        while (AssignEncDecSegments(segments_ptr,
                                    &segment_index,
                                    encDecTasksPtr,
            ...
            lcuStartIndex =
                yLcuStartIndex * picture_width_in_sb + xLcuStartIndex;
            lcuSegmentCount =
            ...
            segmentBandIndex =
                segment_index -
                segmentRowIndex * segments_ptr->segment_band_count;
            segmentBandSize =
                (segments_ptr->lcu_band_count * (segmentBandIndex + 1) +
            ...
            if (picture_control_set_ptr->parent_pcs_ptr
                    ->reference_picture_wrapper_ptr != NULL)
                ((EbReferenceObject *)picture_control_set_ptr->parent_pcs_ptr
                     ->reference_picture_wrapper_ptr->object_ptr)
                    ->average_intensity =
                    picture_control_set_ptr->parent_pcs_ptr
            ...
            if (sequence_control_set_ptr->static_config.improve_sharpness) {
                QpmDeriveWeightsMinAndMax(picture_control_set_ptr, context_ptr);
            }
#endif
            for (y_lcu_index = yLcuStartIndex, lcuSegmentIndex = lcuStartIndex;
                 lcuSegmentIndex < lcuStartIndex + lcuSegmentCount;
                 ++y_lcu_index) {
                for (x_lcu_index = xLcuStartIndex;
                     x_lcu_index < picture_width_in_sb &&
                     (x_lcu_index + y_lcu_index < segmentBandSize) &&
                     lcuSegmentIndex < lcuStartIndex + lcuSegmentCount;
                     ++x_lcu_index, ++lcuSegmentIndex) {
                    sb_index = (uint16_t)(y_lcu_index * picture_width_in_sb +
                                          x_lcu_index);
                    ...
                    lastLcuFlag =
                        (sb_index == sequence_control_set_ptr->sb_tot_cnt - 1)
                            ? EB_TRUE
                            : EB_FALSE;
                    endOfRowFlag = (x_lcu_index == picture_width_in_sb - 1)
                                       ...
                                       : EB_FALSE;
                    lcuRowIndexStart =
                        (x_lcu_index == picture_width_in_sb - 1 &&
                         lcuRowIndexCount == 0)
                            ? y_lcu_index
                            : lcuRowIndexStart;
                    lcuRowIndexCount = (x_lcu_index == picture_width_in_sb - 1)
                                           ? lcuRowIndexCount + 1
                                           : lcuRowIndexCount;
                    mdcPtr = &picture_control_set_ptr->mdc_sb_array[sb_index];
                    context_ptr->sb_index = sb_index;
                    context_ptr->md_context->cu_use_ref_src_flag =
                        (picture_control_set_ptr->parent_pcs_ptr
                             ->use_src_ref) &&
                                (picture_control_set_ptr->parent_pcs_ptr
                                         ->edge_results_ptr[sb_index]
                                         .edge_block_num == EB_FALSE ||
                                 picture_control_set_ptr->parent_pcs_ptr
                                     ->sb_flat_noise_array[sb_index])
                            ? EB_TRUE
                            : EB_FALSE;
                    ...
                    if (picture_control_set_ptr->update_cdf) {
                        ...
                        md_rate_estimation_array +=
                            picture_control_set_ptr->slice_type *
                                TOTAL_NUMBER_OF_QP_VALUES +
                            context_ptr->md_context->qp;
                        ...
                        if (sb_index == 0)
                            picture_control_set_ptr->ec_ctx_array[sb_index] =
                                *picture_control_set_ptr
                                     ->coeff_est_entropy_coder_ptr->fc;
                        else
                            picture_control_set_ptr->ec_ctx_array[sb_index] =
                                picture_control_set_ptr
                                    ->ec_ctx_array[sb_index - 1];
#else
                        if (sb_origin_x == 0)
                            picture_control_set_ptr->ec_ctx_array[sb_index] =
                                *picture_control_set_ptr
                        ...
                        // construct the tables using the latest CDFs : Coeff
                        // Only here ---to check if I am using all the uptodate
                        // CDFs here
                        ...
                        for (candidateIndex = 0;
                             candidateIndex < MODE_DECISION_CANDIDATE_MAX_COUNT;
                             ++candidateIndex)
                            context_ptr->md_context
                                ->fast_candidate_ptr_array[candidateIndex]
                                ->md_rate_estimation_ptr =
                                &picture_control_set_ptr
                                     ->rate_est_array[sb_index];
                    }
#endif
                    // Configure the LCU
                    mode_decision_configure_lcu(context_ptr->md_context,
                                                sb_ptr,
                                                picture_control_set_ptr,
                                                sequence_control_set_ptr,
                    ...
                    if (picture_control_set_ptr->parent_pcs_ptr
                        ...
                        input_picture_ptr =
                            picture_control_set_ptr->parent_pcs_ptr
                                ->enhanced_picture_ptr;

                        // Load the SB from the input to the intermediate SB
                        // buffer
                        uint32_t bufferIndex =
                            (input_picture_ptr->origin_y + sb_origin_y) *
                                input_picture_ptr->stride_y +
                        ...
                        uint32_t sb_height =
                            (sequence_control_set_ptr->luma_height -
                             sb_origin_y) < MAX_SB_SIZE
                                ? sequence_control_set_ptr->luma_height -
                                      sb_origin_y
                                : MAX_SB_SIZE;
                        uint32_t sb_width =
                            (sequence_control_set_ptr->luma_width -
                             sb_origin_x) < MAX_SB_SIZE
                                ? sequence_control_set_ptr->luma_width -
                                      sb_origin_x
                                : MAX_SB_SIZE;
                        uint32_t is_complete_sb =
                            sequence_control_set_ptr->sb_geom[sb_index]
                                .is_complete_sb;

                        if (!is_complete_sb)
                            memset(context_ptr->ss_mecontext->sb_buffer,
                                   0,
                                   MAX_SB_SIZE * MAX_SB_SIZE);
                        for (lcuRow = 0; lcuRow < sb_height; lcuRow++) {
                            EB_MEMCPY(
                                (&(context_ptr->ss_mecontext
                                       ->sb_buffer[lcuRow * MAX_SB_SIZE])),
                                (&(input_picture_ptr->buffer_y
                                       [bufferIndex +
                                        lcuRow * input_picture_ptr->stride_y])),
                                sb_width * sizeof(uint8_t));
                        }

                        context_ptr->ss_mecontext->sb_src_ptr =
                            &(context_ptr->ss_mecontext->sb_buffer[0]);
                        context_ptr->ss_mecontext->sb_src_stride =
                            context_ptr->ss_mecontext->sb_buffer_stride;
...
#else
                        uint32_t me_sb_addr;
                        if (sequence_control_set_ptr->sb_size ==
                            BLOCK_128X128) {
                            uint32_t me_sb_size =
                            ...
                            uint32_t me_sb_addr_0 =
                                me_sb_x + me_sb_y * me_pic_width_in_sb;
                            uint32_t me_sb_addr_1 =
                                (me_sb_x + 1) < me_pic_width_in_sb
                                    ? (me_sb_x + 1) +
                                          ((me_sb_y + 0) * me_pic_width_in_sb)
                                    : me_sb_addr_0;
                            uint32_t me_sb_addr_2 =
                                (me_sb_y + 1) < me_pic_height_in_sb
                                    ? (me_sb_x + 0) +
                                          ((me_sb_y + 1) * me_pic_width_in_sb)
                                    : me_sb_addr_0;
                            ...
                            MeCuResults *me_block_results_0 =
                                &picture_control_set_ptr->parent_pcs_ptr
                                     ->me_results[me_sb_addr_0][0];
                            MeCuResults *me_block_results_1 =
                                &picture_control_set_ptr->parent_pcs_ptr
                                     ->me_results[me_sb_addr_1][0];
                            MeCuResults *me_block_results_2 =
                                &picture_control_set_ptr->parent_pcs_ptr
                                     ->me_results[me_sb_addr_2][0];
                            MeCuResults *me_block_results_3 =
                                &picture_control_set_ptr->parent_pcs_ptr
                                     ->me_results[me_sb_addr_3][0];

                            // Compute average open_loop 64x64 mvs
                            mv_l0_x = ((me_block_results_0->x_mv_l0 +
                                        me_block_results_1->x_mv_l0 +
                                        me_block_results_2->x_mv_l0 +
                                        me_block_results_3->x_mv_l0) >>
                                       2) >>
                                      2;
                            mv_l0_y = ((me_block_results_0->y_mv_l0 +
                                        me_block_results_1->y_mv_l0 +
                                        me_block_results_2->y_mv_l0 +
                                        me_block_results_3->y_mv_l0) >>
                                       2) >>
                                      2;
                            mv_l1_x = ((me_block_results_0->x_mv_l1 +
                                        me_block_results_1->x_mv_l1 +
                                        me_block_results_2->x_mv_l1 +
                                        me_block_results_3->x_mv_l1) >>
                                       2) >>
                                      2;
                            mv_l1_y = ((me_block_results_0->y_mv_l1 +
                                        me_block_results_1->y_mv_l1 +
                                        me_block_results_2->y_mv_l1 +
                                        me_block_results_3->y_mv_l1) >>
                                       2) >>
                                      2;
                        } else {
                            me_sb_addr = sb_index;
                    ...
                    mode_decision_sb(sequence_control_set_ptr,
                                     picture_control_set_ptr,
                                     mdcPtr,
                                     sb_ptr,
                                     sb_origin_x,
                    ...
                    // Configure the LCU
                    EncDecConfigureLcu(context_ptr,
                                       sb_ptr,
                                       picture_control_set_ptr,
                                       sequence_control_set_ptr,
                                       (uint8_t)context_ptr->qp,
...
#if NO_ENCDEC
                    no_enc_dec_pass(sequence_control_set_ptr,
                                    picture_control_set_ptr,
                                    sb_ptr,
                                    sb_index,
                                    sb_origin_x,
...
#else
                    // Encode Pass
                    av1_encode_pass(sequence_control_set_ptr,
                                    picture_control_set_ptr,
                                    sb_ptr,
                                    sb_index,
                                    sb_origin_x,
                    ...
                    if (picture_control_set_ptr->parent_pcs_ptr
                        ...
                        ((EbReferenceObject *)
                            ...
                            (100 * context_ptr->intra_coded_area_sb[sb_index]) /
                            (64 * 64));
                }
        ...
        picture_control_set_ptr->intra_coded_area +=
            (uint32_t)context_ptr->tot_intra_coded_area;
        eb_release_mutex(picture_control_set_ptr->intra_mutex);

        if (lastLcuFlag) {
            // Copy film grain data from parent picture set to the reference
            ...
            EB_MEMCPY(
                ...
                2 * sizeof(int32_t));
            EB_MEMCPY(picture_control_set_ptr->parent_pcs_ptr->av1x
                          ->switchable_restore_cost,
                      context_ptr->md_rate_estimation_ptr
                          ->switchable_restore_fac_bits,
                      3 * sizeof(int32_t));
            EB_MEMCPY(
                picture_control_set_ptr->parent_pcs_ptr->av1x
                    ->wiener_restore_cost,
                ...
                2 * sizeof(int32_t));
            picture_control_set_ptr->parent_pcs_ptr->av1x->rdmult =
                context_ptr->full_lambda;
        }

        if (lastLcuFlag) {
            // Get Empty EncDec Results
            eb_get_empty_object(context_ptr->enc_dec_output_fifo_ptr,
                                &encDecResultsWrapperPtr);
            encDecResultsPtr =
                (EncDecResults *)encDecResultsWrapperPtr->object_ptr;
            encDecResultsPtr->picture_control_set_wrapper_ptr =
                encDecTasksPtr->picture_control_set_wrapper_ptr;
            // CHKN these are not needed for DLF
            encDecResultsPtr->completed_lcu_row_index_start = 0;
            encDecResultsPtr->completed_lcu_row_count =
                ((sequence_control_set_ptr->luma_height +
                  sequence_control_set_ptr->sb_size_pix - 1) >>
                 lcuSizeLog2);
            // Post EncDec Results
...
void av1_add_film_grain(EbPictureBufferDesc *src, EbPictureBufferDesc *dst,
    ...
    fgn_copy_rect(
        src->buffer_y + ((src->origin_y * src->stride_y + src->origin_x)
                         << use_high_bit_depth),
        src->stride_y,
    ...
    luma = dst->buffer_y + ((dst->origin_y * dst->stride_y + dst->origin_x)
                            << use_high_bit_depth);
    cb = dst->buffer_cb +
         ((dst->stride_cb * (dst->origin_y >> chroma_subsamp_y) +
           (dst->origin_x >> chroma_subsamp_x))
