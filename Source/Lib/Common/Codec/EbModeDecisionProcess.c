...
EbErrorType mode_decision_context_ctor(
    ...
    *context_dbl_ptr = context_ptr;
    ...
    pictureBufferDescInitData.buffer_enable_mask = PICTURE_BUFFER_DESC_LUMA_MASK;
    ...
    // Video Buffers
        ...
        return_error = eb_picture_buffer_desc_ctor(
        (EbPtr*)&(context_ptr->prediction_ptr_0),
        (EbPtr)&pictureBufferDescInitData);

        return_error = eb_picture_buffer_desc_ctor(
            (EbPtr*)&(context_ptr->prediction_ptr_1),
            (EbPtr)&pictureBufferDescInitData);
#endif
    return EB_ErrorNone;
}
    ...
    uint32_t                   segment_index)
{
    ...
    picture_control_set_ptr->parent_pcs_ptr->switchable_motion_mode =
        picture_control_set_ptr->parent_pcs_ptr->switchable_motion_mode || picture_control_set_ptr->parent_pcs_ptr->pic_obmc_mode;
