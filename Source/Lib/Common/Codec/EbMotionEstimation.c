...
static void PU_HalfPelRefinement(
    ...
    // Compute SSD for the best full search candidate
    if (context_ptr->fractional_search_method == SSD_SEARCH) {
        *pBestSsd = spatial_full_distortion(
...
#if NSQ_TABLES_NREF
    is_nsq_table_used = ((picture_control_set_ptr->enc_mode <= ENC_M0 && picture_control_set_ptr->is_used_as_reference_flag) || (picture_control_set_ptr->enc_mode == ENC_M0 && picture_control_set_ptr->sc_content_detected))
#else
    is_nsq_table_used = (picture_control_set_ptr->enc_mode == ENC_M0 || (picture_control_set_ptr->enc_mode == ENC_M0 && picture_control_set_ptr->sc_content_detected))
#endif
                    ...
                    if (!disable_angular_prediction) {
                        for (angle_delta_counter = 0;
                            ...
                            intra_prediction_open_loop(p_angle,
                                                       ...
                                                       context_ptr);
                            // Distortion
                            ois_blk_ptr[ois_intra_count].distortion = nxm_sad_kernel(  // Always SAD without weighting
                                        &(input_ptr->buffer_y
                                              [(input_ptr->origin_y +
                                                cu_origin_y) *
                                                   input_ptr->stride_y +
