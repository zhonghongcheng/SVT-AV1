...
EB_API EbErrorType eb_init_encoder(EbComponentType *svt_enc_component)
{
    ...
    ************************************/
    ...
    for (instance_index = 0; instance_index < enc_handle_ptr->encode_instance_total_count; ++instance_index) {
        ...
        quarterPictureBufferDescInitData.buffer_enable_mask = PICTURE_BUFFER_DESC_FULL_MASK;
#else
...
#if HME_LEVEL_O_CHROMA
        sixteenthPictureBufferDescInitData.buffer_enable_mask = PICTURE_BUFFER_DESC_FULL_MASK;
#else
