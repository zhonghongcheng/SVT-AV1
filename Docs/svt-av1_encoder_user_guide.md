...
| **PaletteMode** | -palette | [0 - 6] | -1 | Enable Palette mode (-1: DEFAULT Mode(ON at level6 when SC is detected), 0: OFF 1: ON Level 1, ...6: ON Level6 ) |
| **OlpdRefinement** | -olpd-refinement | [0 - 1] | -1 | Enable open loop partitioning decision refinement (-1: DEFAULT Mode(ON for M0, no SC, OFF otherwise), 0: OFF 1: ON for M0, error otherwise ) |
